...
|---|---|---|
| Optical planes and workspaces | `TelescopeAperture` separates prepared geometry from transitional legacy path state; caller-owned `PupilFunction`, `ElectricField`, and `IntensityMap` products use separate propagation and spatial-filter workspaces, and the telescope has no cadence or exposure field | Complete migration of WFS, science, and legacy convenience paths to explicit products, then remove transitional telescope path state |
...
| WFS composition | Sensor-specific state commonly still combines optical propagation, rate buffers, detector acquisition, calibration, and estimation. Shack–Hartmann same-wavelength spectral, extended-source, and asterism components are now summed before one detector acquisition; distinct wavelength grids fail closed | Prepared optical front ends produce photon-arrival-rate products with an explicit spatial measure, consumed by independent detector acquisition and estimators, including multi-plane/multi-detector cases and explicit native-to-detector spectral-grid mappings |
...
| Science optics and NCPA | Direct prepared propagation preserves photon-rate metadata, while telescope-owned convenience PSF storage, sampled NCPA primitives, and a prepared PROPER handoff remain | Caller-owned native or explicitly converted external photon-arrival-rate products feed independent detector acquisition; later path composition adds explicit NCPA visibility and scheduling |
