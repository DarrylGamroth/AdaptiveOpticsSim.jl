...
The superseded generic single-optic runtime, composite-optic wrapper, and packed
command-layout API have been removed after their numerical oracles migrated.
New HIL architecture MUST build on the independent Plant owners rather than
reintroducing those abstractions for source compatibility. The generic
`slopes` accessor remains transitional and MUST NOT be extended to sensors
whose measurements are not physically slopes.
...
| Time and radiometry | Timed atmospheres receive explicit absolute model time. The WFS optics stage produces declared photon-rate or dimensionless products without elapsed time, exact event lifecycles apply each detector interval once, and command endpoints retain requested/scheduled/application timestamps. The serial event loop uses integer-nanosecond schedules and advances one shared atmosphere once for every due optical timestamp | Add injected execution-clock pacing while preserving the separate plant-time, optical-rate, detector-integration, command-effective-time, and execution-lateness contracts |
...
| WFS composition | Shack–Hartmann separates microlens optics, layout/calibration, acquisition, and estimation. Pyramid and Bi-O-edge expose physically distinct focal-plane front ends over shared prepared modulation. Zernike separates its phase spot and pupil relay; Curvature separates its two branch planes and supports independent or packed detector acquisition. LiFT separates a shareable forward plan, per-owner prepared forward workspace/input/output, explicit observation domain, and independently prepared iterative estimator/result without detector ownership or `AbstractWFS` inheritance | Preserve the completed Gate 0 staged contract while composing independently scheduled acquisitions |
| Shared plant and paths | One prepared telescope/atmosphere feeds reusable path owners, independent acquisition owners, independently commanded controllable optics, and a deterministic linear reduced-order direct-measurement model. The serial event loop composes unequal science/NGS/LGS schedules, shared-result fan-out, exact atmosphere-epoch materialization, stable owner RNGs, right-continuous command application, and reduced-only path bypass without retaining run-length event lists | Add prepared execution groups and explicit CPU/GPU placement without weakening the serial oracle |
...
| Controllable optics | Named independent optics and endpoints have versioned schemas, bounded late/future/silence policy, explicit pupil or atmospheric-conjugate placement, common or selected-path visibility, NGS/LGS footprint coupling, additive co-conjugated application, explicit transactions, zero-copy prepared controller-output routing, and native DM MCAO/MOAO composition | Add model-specific physical device dynamics and prepared execution placement without changing optical identity or command ownership |
| Science optics and NCPA | Prepared direct imaging owns explicit pupil/field/output/workspace products, supports same-grid incoherent composition and spectral bundles, and reuses one rate image across independent detector acquisitions. Native `NCPA` and `OPDMap` effects have explicit placement, common or selected-path visibility, defensive run ownership, canonical additive/replacement ordering, and a typed external path-executor handoff | Caller-owned native or explicitly converted external photon-arrival-rate products feed independently scheduled detector acquisition; asynchronous external-executor scheduling remains a later HIL concern |
| Product fidelity | One prepared acquisition seam binds full-optical, command-responsive reduced-order, or nonresponsive unchanged/copy/bounded-replay providers to an invariant caller-owned product contract. Core validates metadata and result identity and bypasses otherwise unused full-optical path execution. The scheduled built-in reduced-order surface is validated only for its declared deterministic linear modal direct-measurement envelope | Carry the same product contract into scheduled descriptors, leases, ports, and overload behavior; add production-shaped load evidence and only those additional model envelopes that receive explicit validity evidence |
