...
| Detector acquisition | Detector families plus a prepared global-shutter begin/accumulate/nondestructive-read/close/readout/readiness lifecycle over exact plant time; scheduled HgCdTe ramps sample evolving charge, while direct `capture!` retains a separately labeled post-exposure synthesized convenience | Add rolling row/band generators and frame-transfer acquisition/readout overlap while preserving the prepared rate boundary, detector ordering, and current exact global-shutter oracle |
