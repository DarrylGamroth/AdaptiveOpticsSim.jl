...
|---|---|---|
| Optical planes and workspaces | `TelescopeAperture` now separates prepared geometry from transitional legacy path state; caller-owned `PupilWavefront`, `ElectricField`, and `IrradiancePlane` products use separate propagation and spatial-filter workspaces | Complete migration of atmosphere, WFS, and legacy convenience paths to the explicit products, then remove transitional telescope path state and cadence |
