...
| Shared plant and paths | One prepared telescope/atmosphere feeds reusable path owners, independent acquisition owners, and independently commanded controllable optics. The serial event loop composes unequal science/NGS/LGS schedules, shared-result fan-out, exact atmosphere-epoch materialization, stable owner RNGs, and right-continuous command application without retaining run-length event lists | Add production reduced-order command-responsive paths, prepared execution groups, and explicit CPU/GPU placement without weakening the serial oracle |
