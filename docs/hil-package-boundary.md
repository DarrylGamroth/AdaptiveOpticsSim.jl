...
| Time and radiometry | Timed atmospheres now receive explicit elapsed or absolute model time; the transitional closed-loop adapter still chooses telescope sampling time, and several optical/WFS radiometric normalizations precede detector exposure scaling | Remove telescope cadence from the remaining runtime/optical contracts, produce declared photon-arrival-rate products, and apply acquisition elapsed time exactly once |
