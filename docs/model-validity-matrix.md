...
| `MV-03` | WFS optical and observation families | `A, R, G, P, M` — [`wfs_common_and_parity.jl`](../test/testsets/wfs_common_and_parity.jl), [`shack_hartmann_and_sources.jl`](../test/testsets/shack_hartmann_and_sources.jl), [`pyramid_bi_o_edge_and_lgs.jl`](../test/testsets/pyramid_bi_o_edge_and_lgs.jl), [`zernike_and_curvature.jl`](../test/testsets/zernike_and_curvature.jl), [`wfs_lift.jl`](../test/testsets/wfs_lift.jl), [`backend_optional_common.jl`](../test/backend_optional_common.jl), and the [LiFT AOS/AOC profile](../benchmarks/results/platform/2026-09-22-lift-aoc-adapter-profile.toml) | Shack–Hartmann, Pyramid, Bi-O-edge, Zernike, and Curvature coverage ends at physical optical formation and complete detector observations; maintained operational estimation is qualified in the registered FGA/JFG fixtures. LiFT physical prediction and analytic Jacobians remain in AOS; registered AdaptiveOpticsCalibration v0.10 `PhaseRetrieval` owns the direct inverse API, policies, solves, products, and diagnostics, exercised with the live model on CPU, CUDA, and AMDGPU. CPU explicit-workspace inverse execution is zero allocated; direct GPU inverse execution allocates and synchronizes and is not captured-graph execution. Weighted-centroid processing is not an AOS capability or validity claim. Instrument-specific optical gain and non-common-path calibration remain external. | strong for declared numerical models and ownership boundaries |
