...
| `MV-08` | Static complete-frame algorithm graphs | `A, P, M` — [`algorithm_graphs.jl`](../test/testsets/algorithm_graphs.jl) | Concrete nodes, exact port contracts, direct/delayed links, TOML loading, sparse parameters, preparation, reset, model time, failure state, and zero warmed allocation for covered paths. User nodes need their own evidence. | strong |
| `MV-09` | External-RTC lockstep boundary | `A, P, M` — HIL testsets in [`algorithm_graphs.jl`](../test/testsets/algorithm_graphs.jl) | One complete host frame followed by one finite same-sequence command, explicit host/device copies, reset and failure containment. No transport, wall-clock pacing, or fixed-arrival latency claim. | strong for transport-neutral lockstep |
| `MV-10` | REVOLT Classic and Copper graph examples | `R, P, M` — [`revolt_classic_hil.toml`](../examples/graphs/revolt_classic_hil.toml), [`revolt_copper_hil.toml`](../examples/graphs/revolt_copper_hil.toml), executable tests in [`algorithm_graphs.jl`](../test/testsets/algorithm_graphs.jl) | Complete simulated detector-frame paths with atmosphere and PDM command input. The HSDM277 influence function is provisional and the examples are not instrument qualification. | executable; scientific qualification partial |
