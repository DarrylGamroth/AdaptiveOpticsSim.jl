...
references use the legacy matched-step convention in which telescope
`sampling_time` participates in atmosphere advancement and several optical/WFS
signal/count normalizations before detector exposure scaling. Until `HIL-RAD-001` and
`HIL-ATM-002` are validated, those references do not establish independent
multi-rate radiometry or prove that elapsed time is applied exactly once for
unequal telescope-step and detector-exposure values. Likewise, current
