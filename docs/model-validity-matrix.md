...
Across `MV-03` through `MV-09` and `MV-13`, current optical and runtime
references retain a legacy matched-step scenario in which the runtime adapter
chooses telescope `sampling_time` as its explicit atmosphere duration and
several optical/WFS signal/count normalizations precede detector exposure
scaling. The atmosphere API no longer owns or infers that cadence, but these
references still do not establish independent multi-rate radiometry or prove
that elapsed time is applied exactly once for unequal plant-step and
detector-exposure values. That claim remains gated by `HIL-RAD-001` and the
multi-rate scheduling work. Likewise, current
polychromatic array accumulation is not evidence that wavelength-dependent
planes share a common physical coordinate grid; that claim requires
`HIL-PLANE-004` and `HIL-RAD-002` evidence.
