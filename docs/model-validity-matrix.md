...
| `MV-08` | Static complete-frame algorithm graphs | `A, P, M` — [`algorithm_graphs.jl`](../test/testsets/algorithm_graphs.jl) | Concrete nodes, exact port contracts, direct/delayed links, TOML loading, sparse parameters, preparation, capacity-one completion-ticket ownership, reset, model time, failure state, and zero warmed allocation for covered paths. User nodes need their own evidence; multiple in-flight frames require separately owned slots. | strong |
