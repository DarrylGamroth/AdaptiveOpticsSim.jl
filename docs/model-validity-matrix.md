...
| `MV-10` | REVOLT Classic and Copper graph examples | `R, P, M` — [`revolt_classic_hil_coordinate_gaussian.toml`](../examples/graphs/revolt_classic_hil_coordinate_gaussian.toml), [`revolt_copper_hil_coordinate_gaussian.toml`](../examples/graphs/revolt_copper_hil_coordinate_gaussian.toml), executable tests in [`algorithm_graphs.jl`](../test/testsets/algorithm_graphs.jl), and optional REVOLT Classic [pyRTC process validation](../examples/integrations/pyrtc/test_revolt_classic_hil.jl) | Complete simulated detector-frame paths with atmosphere and PDM command input. The Classic model self-calibrates and closes through pyRTC with the maintained dimensions. The HSDM277 influence function is provisional and the examples are not instrument qualification. | executable; scientific qualification partial |
