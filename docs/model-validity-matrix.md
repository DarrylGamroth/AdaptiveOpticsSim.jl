...
| `MV-06` | Tomography | `A, G, P, M` — [`tomography.jl`](../test/tomography.jl), [`gpu_builder_contract.jl`](../scripts/gpu_builder_contract.jl), the [CPU/GPU cold-build profile](../benchmarks/results/platform/2026-09-23-aoc-v016-model-tomography-profile.toml) | AOS owns guide-star/layer geometry, sampled DM fitting, and cold model- and interaction-matrix composition. Registered AOC v0.16 owns numerical Cxx/Cox assembly on CPU, CUDA, and AMDGPU; AOS supplies selected transformed physical coordinates. These are allocating, synchronized cold-build paths, not captured-graph or RTC execution. FilterGraphAlgorithms/JuliaFilterGraph owns per-frame matrix application. Instrument MCAO/MOAO calibration remains scenario work. | implemented on tested targets; external equivalence partial |
