...
| HIL-EXEC-006 | 6 | Prepared execution keeps small fixed stage pipelines specialized while representing large path/endpoint registries with bounded compilation and code growth verified against topology scale | Core | partial | partial | The normative [Gate 6 clauses](execution-and-placement.md#gate-6-prepared-cpu-requirements) distinguish large-registry representation from small pipeline specialization and require cold-growth evidence. [`PreparedPlantEventLoop`](../../src/plant/event_composition.jl) now stores `PreparedPathExecutionGroup` values in a topology-size-invariant homogeneous `Memory` registry behind the existing no-inline path barriers, while each group retains its concretely typed `PreparedPathExecutor`. [`plant_event_composition.jl`](../../test/testsets/plant_event_composition.jl) checks the homogeneous container and inferred access boundary. Remaining schedule-free tuple recursion plus preparation, first-use, and generated-code growth over the declared topology matrix remain #98; no MORFEO/NFIRAOS capacity claim is made. |
