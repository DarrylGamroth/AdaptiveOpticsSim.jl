...
| HIL-CPU-001 | 6 | Prepared CPU execution groups have single-writer workspaces, explicit thread budgets, independently callable executor seams, and a deterministic serial fallback | Core | partial | partial | The normative [Gate 6 clauses](execution-and-placement.md#gate-6-prepared-cpu-requirements) now separate group topology, ownership, direct executors, serial fallback, thread budgets, and deployment placement. [`PreparedPathExecutionGroup`](../../src/plant/event_composition.jl) gives every canonically ordered scheduled path one bounded run-immutable ownership record with its path-local executor, workspace/product references, RNG owner, and exact compatible acquisition membership; the serial event loop traverses consumers through those groups. [`plant_event_composition.jl`](../../test/testsets/plant_event_composition.jl) validates stable path/group ordinals and membership under reversed declarations, homogeneous storage, exact shared-result fan-out, inference, structured missing-schedule rejection, deterministic replay, and the retained warmed orchestration ceiling. Independently callable batch/group executors and explicit Julia/FFT/BLAS budgets remain #96 and #97; long-lived HIL agents and rings remain Gate 8. |
