...
| HIL-CMD-001 | 4 | Every independently timed controllable optic or segment has an independent virtual-time command endpoint | Plant | partial | partial | [`definitions.jl`](../../src/plant/definitions.jl) provides stable `ControllableOpticID` and `CommandEndpointID` values, one cold `ControllableOpticDefinition` per physical device, one-or-more explicitly owned endpoint schemas, declaration-order-independent lookup, and duplicate-owner rejection. [`command_admission.jl`](../../src/plant/command_admission.jl) now prepares one independently timed standalone endpoint owner per selected schema, with its own state, capacity, history, calendar, ordinal, and disposition workspace. [`plant_topology.jl`](../../test/testsets/plant_topology.jl) and [`plant_command_admission.jl`](../../test/testsets/plant_command_admission.jl) validate topology and endpoint isolation. Automatic preparation for every declared plant optic plus physical application/optical composition remain later Gate 4 slices; the minimal HIL port arrives in Gate 4A. |
| HIL-CMD-002 | 4 | Validation, admission, effective time, application, hold, and terminal model disposition are distinct | Plant | partial | partial | [`command_schemas.jl`](../../src/plant/command_schemas.jl) keeps non-mutating payload validation separate. [`command_admission.jl`](../../src/plant/command_admission.jl) implements distinct presentation, admission, pending, application-ready claim, and terminal-disposition transitions. [`command_application.jl`](../../src/plant/command_application.jl) separately owns the effective held value and transactionally applies absolute replacement or incremental addition before publishing one applied/rejected/failed disposition. Requested effective, scheduled, admission, application-ready, application, silence, and terminal timestamps remain distinct. Focused [admission](../../test/testsets/plant_command_admission.jl) and [application](../../test/testsets/plant_command_application.jl) tests cover exactly-once credit, state preservation on rejection/failure, inference, zero warmed CPU allocation, and direct CUDA/AMDGPU-resident array storage. Physical optic mutation, device response, plant-event composition, and atomic multi-optic latch remain later slices; atomicity is never inferred from placement. |
| HIL-CMD-003 | 4 | Future, late, rejected, and superseded commands use bounded explicit policies and never backdate plant state | Plant | partial | partial | The standalone prepared endpoint implements fixed-capacity time ordering; accepted-sequence history; explicit duplicate/stale/reordered/skipped, future, late, capacity, and supersession outcomes; retry after nonadmission; apply-now lateness; and no-backdating. Absolute supersession emits one terminal credit per displaced pending presentation; incremental supersession is rejected as lossy. Effective application requires the exact immutable scheduled timestamp; a later claim receives terminal `:missed_application_timestamp` failure instead of backdating state. A silence transition likewise requires its exact deadline. Focused tests validate deterministic scheduled-time/endpoint-ordinal/local-sequence order, capacity invariants, retry semantics, terminal outcomes, and endpoint-owned terminal metadata. Plant-event composition and fixed-arrival HIL overload evidence remain missing, so implementation and evidence stay partial at the system requirement level. |
| HIL-CMD-004 | 4 | Every endpoint prepares a versioned core plant command schema covering payload type/shape, units, basis/calibration, absolute or incremental semantics, bounds, plant-effective-time policy, and duplicate/reordering behavior | Plant | partial | partial | `PlantCommandSchema` covers stable schema/version identity, exact scalar or backend-neutral array type/shape, units/sign, basis/revision, absolute/incremental semantics, uniform/unbounded values, and value/sequence/effective-time policies. `prepare_command_endpoint` binds one selected schema to fixed admission storage, ordering, and backend policy; `CommandApplicationState` binds its explicit initial effective value and any required safe value to the same exact endpoint. Focused schema, admission, and application tests validate those bindings. `prepare_plant` does not yet prepare every declared optic endpoint or bind effective command state to a physical optic, so the requirement remains partial. HIL boundary descriptors, session correlation, timestamp mapping, leases, and returned outcome credit remain `HIL-PORT-005`. |
| HIL-CMD-005 | 4 | Every endpoint declares replayable plant-time command-silence hold/safe/fail behavior, including the age origin and exact equal-time reset policy | Plant | partial | partial | `CommandSilencePolicy` declares indefinite hold or a safe/fail action, admission/application age origin, and a required positive plant-time timeout for safe/fail. [`command_application.jl`](../../src/plant/command_application.jl) prepares copied safe state, tracks exact origins, schedules one transition per unchanged origin, gives an equal-time due command precedence, and terminally drains/fails an endpoint when required. [Focused tests](../../test/testsets/plant_command_application.jl) cover first-command baselines, both age origins, safe-value ownership, rearming, equal-time order, pending drain, inference, warmed allocation, and direct CUDA/AMDGPU-resident safe state. Automatic preparation/composition for every declared plant endpoint remains missing, so the system row stays partial; operational ingress liveness remains `HIL-LIFE-002`. |
...
| HIL-OPT-001 | 4 | Co-conjugated DMs remain independent devices even when prepared as one optical execution group | Plant + Optics | partial | partial | [`definitions.jl`](../../src/plant/definitions.jl) and [`plant_topology.jl`](../../test/testsets/plant_topology.jl) now preserve two co-conjugated test-model devices as distinct physical declarations with disjoint endpoint ownership regardless of declaration order. [`controllable_optics.jl`](../../src/optics/controllable_optics.jl) and [runtime tests](../../test/testsets/control_and_runtime.jl) retain the additive composite numerical oracle. Prepared independent surface application and derived execution grouping remain missing. |
