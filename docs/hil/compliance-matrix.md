...
| HIL-PATH-001 | 2 | Prepared paths own mutable propagation workspace; acquisitions separately own WFS, detector, readout, and publication state | Core | partial | partial | [`preparation.jl`](../../src/plant/preparation.jl) now freezes each source and builds one concrete `PreparedPathExecutor` with an explicit path-local input/result and prepared optical execution owner; `PreparedAcquisitionOwner` values borrow the exact result read-only while owning independent detector/readout, optional WFS estimator, observation, and measurement state. [`plant_preparation.jl`](../../test/testsets/plant_preparation.jl) exercises direct-science and diffractive Shack–Hartmann owners, exact compatible fan-out to unequal-exposure detectors, inference, and zero warmed CPU allocation. The [optional backend matrix](../../test/backend_optional_common.jl) executes the prepared direct-science/fan-out boundary device-resident in the `399/399` CUDA and `409/409` AMDGPU hardware targets with scalar indexing disabled. Selected-set atmosphere materialization arrives in #30; scheduling and publication ownership remain later work, so the row stays partial. |
