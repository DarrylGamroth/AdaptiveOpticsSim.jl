...
generated-code growth, prepared parallel CPU groups, integrated GPU execution,
fixed-arrival HIL latency, and NFIRAOS/MORFEO qualification remain Gates 6-8.
...
The path-group lifecycle and CPU thread-budget portion is implemented and
validated by the focused one- and four-thread `gate6` test configurations.
Topology-scale preparation, first-use, and generated-code evidence remains the
separate `HIL-EXEC-006` closure.
