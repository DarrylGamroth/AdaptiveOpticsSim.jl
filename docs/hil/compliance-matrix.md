...
| HIL-DET-001 | 3 | Exposure, optical samples, rolling shutter, frame transfer, nondestructive reads, readout, presampling detector response, charge coupling, and complete-product publication have explicit composable event semantics; the realized response has a validated interior MTF where supported and explicit finite-frame boundary behavior | Core | partial | partial | [`detector_acquisition_events.jl`](../../src/plant/detector_acquisition_events.jl) implements the global-shutter begin, exact half-open interval, nondestructive-read, close, readout-complete, and readiness lifecycle over one prepared detector/rate-product binding. [`plant_detector_transitions.jl`](../../test/testsets/plant_detector_transitions.jl) validates exact boundaries, reject-on-busy retriggers, atomic transition errors, conventional whole-frame parity through presampling response and charge coupling, unchanged MTF/finite output, inference, zero warmed CPU allocation, and device-resident accelerator execution through the optional matrix. Rolling row/band generators and frame-transfer overlap remain #49, so the full cross-family requirement stays partial. |
...
| HIL-TIME-001 | 3 | Nonnegative integer-nanosecond plant timestamps and durations use checked arithmetic, while event phase, prepared ordinal, and per-generator occurrence determine total equal-time ordering | Core | partial | partial | [`plant/time.jl`](../../src/plant/time.jl) defines distinct checked timestamp/duration values, a signed checked time offset, positive periodic recurrence, and a total logical event key. [`plant/scheduling.jl`](../../src/plant/scheduling.jl) assigns prepared phase/ordinal registry order and rejects any claimed, activated, or rescheduled key that would regress the total order while allowing a strictly later occurrence at the same timestamp. [`plant/triggers.jl`](../../src/plant/triggers.jl) additionally enforces source-before-delivery order and rejects physical backdating across its prepared fan-out. [`detector_acquisition_events.jl`](../../src/plant/detector_acquisition_events.jl) now uses exact integer boundaries as the global-shutter lifecycle authority and converts only positive interval durations at the detector-physics seam. The focused time, scheduler, trigger, and detector-transition suites cover arithmetic, type separation, simultaneous ordering, trigger chronology, half-open exposure boundaries, inference, and warmed allocation. Rolling/frame-transfer scheduling and composed Gate 3 orchestration remain #49. |
| HIL-TRIG-001 | 3 | A prepared trigger-source and distribution topology maps nominal edges to per-acquisition delivery times with explicit phase/skew, jitter, bounded non-overtaking delivery, dropped/duplicate-edge policies, and separate physical exposure and timestamp-label semantics | Core | partial | partial | [`plant/triggers.jl`](../../src/plant/triggers.jl) implements stable typed source/link/consumer/fault identities, canonical finite acyclic fan-out preparation, exact finite source-sequence traces, persistent phase steps, per-sequence jitter and label offsets, correlated source faults, isolated downstream link faults, explicit drops/duplicates, deterministic occurrence order, conservative in-flight-capacity rejection, fixed pending storage, separate nominal/delivered/reported records, fault provenance, and allocation-free caller-owned delivery removal. [`plant_triggers.jl`](../../test/testsets/plant_triggers.jl) validates declaration reordering, nested fan-out, chronology, capacity, fault scope, 100,000-cycle constant storage, inference, and zero warmed allocation. The global-shutter detector transition target now exists, but mapping each delivered consumer edge into that target and composing it with the common scheduler remains #49; stochastic traces, camera-internal oscillators, rings, and execution-clock pacing are not claimed. |
