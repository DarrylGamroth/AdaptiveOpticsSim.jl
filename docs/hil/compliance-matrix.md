...
| HIL-RAD-001 | 0 | Physical detector-facing optical products are photon-arrival-rate products with declared units and density-versus-cell-integrated measure; telescope and optical front ends apply no elapsed-time factor, while detector acquisition integrates its explicit whole-exposure or incremental duration exactly once and never creates photons through declared spatial response and pixel integration; finite-support loss is explicit | Core | implemented | validated | Physical and normalized source radiometry in [`source.jl`](../../src/optics/source.jl) propagates through rate- or dimensionless-normalized fields and intensity maps without a telescope-time factor. [`DetectorAcquisitionPlan`](../../src/detectors/detector_acquisition.jl) validates a detector-facing map and prepares density-to-cell scaling and declared-channel QE; repeated whole or incremental capture applies a non-null presampling response before physical-pixel integration and applies explicit exposure exactly once. The prepared Shack–Hartmann front end writes cell-integrated detector-plane photon-rate mosaics without elapsed time; acquisition then applies presampling response/MTF, physical-pixel integration, QE, explicit duration, stochastic response, and readout. Distinct wavelength grids remain separate rate products in an `OpticalProductBundle`. Finite response application uses zero extension, so signal leaving detector support is lost and edge samples are never amplified by clamping. [`core_optics.jl`](../../test/testsets/core_optics.jl), [`detectors.jl`](../../test/testsets/detectors.jl), the prepared [`WFS stage contracts`](../../test/testsets/wfs_stage_contracts.jl), [`shack_hartmann_and_sources.jl`](../../test/testsets/shack_hartmann_and_sources.jl), [`pyramid_bioedge_and_lgs.jl`](../../test/testsets/pyramid_bioedge_and_lgs.jl), and the [optional backend matrix](../../test/backend_optional_common.jl) cover unequal exposures, non-unit duration, density and cell-integrated fixtures, rejection, non-commuting response order, spectral bundling, component accumulation, edge behavior, sampled-QE channel selection, prepared incremental GPU acquisition, and zero warmed allocation. The physical Shack–Hartmann fixture proves one unchanged rate product fans out to unequal detector durations and that QE and duration are each applied exactly once. |
| HIL-RAD-002 | 0 | Spectral/source planes are accumulated elementwise only with compatible physical grids and declared incoherent intensity/rate semantics; otherwise a typed bundle or explicit prepared mapping is required | Core | implemented | validated | [`prepare_incoherent_sum`](../../src/optics/planes.jl) validates immutable metadata once and [`accumulate_intensity!`](../../src/optics/planes.jl) performs prepared allocation-free accumulation; [`OpticalProductBundle`](../../src/optics/planes.jl) preserves incompatible products without implicit conversion or resampling. Unspecified spectral coordinates cannot authorize an intensity sum. The prepared Shack–Hartmann front end forms each distinct wavelength on its native angular grid and retains the results as a bundle; passing that source to one output fails before execution. The legacy single-product `measure!` convenience path remains restricted to a common wavelength grid. [`Optical radiometry and combination contracts`](../../test/testsets/core_optics.jl) covers compatible accumulation and rejection, while [`wfs_stage_contracts.jl`](../../test/testsets/wfs_stage_contracts.jl) validates wavelength-dependent Shack–Hartmann sampling, component weights, bundle retention, and single-output rejection. |
...
| HIL-WFS-002 | 0 | WFS implementations compose prepared optical-front-end, detector-acquisition, and estimator stages through dispatch over caller-owned products without HIL clock, scheduling, queue, or transport dependencies | Core | partial | partial | [`stage_contracts.jl`](../../src/wfs/stage_contracts.jl) defines typed caller-owned observations/measurements and six narrow prepared dispatch functions for optical formation, acquisition, and estimation, plus explicit acquired/direct measurement traits and structured preparation errors. Shack–Hartmann is the first physical family implementation: [`stages.jl`](../../src/wfs/shack_hartmann/stages.jl) binds caller-owned pupil/field, rate product, observation, and measurement storage without a clock, queue, transport, or runtime graph dependency. [`wfs_stage_contracts.jl`](../../test/testsets/wfs_stage_contracts.jl) covers exact binding, inference, stage independence, and warmed allocation; the optional backend matrix covers device residency with scalar indexing disabled. Pyramid/BioEdge, Zernike/Curvature, and LiFT remain work issues [#5](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/5)–[#7](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/7), so the cross-family requirement remains partial. |
| HIL-WFS-003 | 0 | Optical front ends produce one or more detector-plane photon-arrival-rate products with an explicit spatial measure; presampling response, physical-pixel integration, QE and explicit elapsed-time integration, coupling, stochastic response, and readout follow optical formation and precede estimation, with one or multiple detector observations supported; the realized response's interior MTF remains available as a derived diagnostic | Core | partial | partial | The generic [`stage contract`](../../src/wfs/stage_contracts.jl) requires detector-plane photon-arrival-rate products and supports static one/multiple-plane and one/multiple-observation composition. The prepared Shack–Hartmann implementation now forms cell-integrated rate mosaics or wavelength-native bundles before detector acquisition; [`wfs_stage_contracts.jl`](../../test/testsets/wfs_stage_contracts.jl) proves unequal-exposure fan-out and a non-commuting presampling-response/pixel-integration fixture before typed estimation. [`detectors.jl`](../../test/testsets/detectors.jl) verifies response, pixel integration, QE, exposure, and readout behavior, including detector MTF diagnostics. The remaining family-owned front ends and estimators, including Curvature's separate/packed acquisition forms, remain work issues [#5](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/5) and [#6](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/6). |
| HIL-WFS-004 | 0 | Shack-Hartmann composes an independent microlens-array optical primitive, detector acquisition, and replaceable spot estimator while preserving geometric, diffractive, spectral, LGS, and asterism behavior | Core | implemented | validated | [`MicrolensArrayParams`, `MicrolensArray`, and `MicrolensArrayWorkspace`](../../src/wfs/shack_hartmann/setup.jl) separate immutable optical configuration from prepared propagation state; layout, calibration, acquisition, and estimator storage are distinct components. [`stages.jl`](../../src/wfs/shack_hartmann/stages.jl) consumes a caller-owned pupil/field, writes photon-rate mosaics or spectral bundles without telescope mutation or cadence, and composes independent detector acquisition and direct/acquired estimation. [`wfs_stage_contracts.jl`](../../test/testsets/wfs_stage_contracts.jl) checks legacy-equivalent diffractive output, geometric direct measurement, LGS elongation and sodium profiles, asterism accumulation, spectral bundles, response ordering, exact-once exposure, caller-owned mutation, and zero warmed CPU allocation. The optional backend matrix executes the physical stages on device-resident arrays with scalar indexing disabled. |
...
| HIL-WFS-008 | 0 | Geometric and reduced-order WFS policies avoid unused diffractive workspace and declare whether they produce an approximate photon-arrival-rate product for detector processing or intentionally produce a direct measurement | Core | partial | partial | `AcquiredObservationPath()` and `DirectMeasurementPath()` establish the acquired-observation versus intentional-direct-measurement distinction. Geometric `ShackHartmannWFS` now prepares no microlens FFT or acquisition workspace and its explicit-pupil direct estimator is warmed-allocation-free. A package-wide fidelity declaration and migration of the remaining reduced-order WFS modes remain future work across the [Gate 0 series](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/1), so the cross-family requirement remains partial. |
| HIL-WFS-009 | 0 | Every migrated stage preserves deterministic CPU results, maintained CUDA and AMDGPU parity, device residency, warmed allocation budgets, and comparable steady-state latency within predeclared absolute and relative gates | Core | partial | partial | The prepared Shack–Hartmann optical, acquisition, and estimation stages preserve the frozen CPU behavior and are allocation-free after warmup; geometric construction omits unused diffractive storage. The shared [optional backend matrix](../../test/backend_optional_common.jl) directly executes the physical Shack–Hartmann stages on device-resident CUDA and AMDGPU arrays with scalar indexing disabled. The committed [Pre-HIL 7 Gate 0 artifact](../../benchmarks/results/gate0/2026-07-16-pre-hil-07-shack-hartmann-stage.toml) records zero steady-state allocation, 84.9–86.3 μs median and 107–130 μs p99 across three 10,000-sample repetitions for `G0-PERF-05`, with every absolute and relative gate passing. This advances one family, but the requirement remains partial until the other WFS migrations and final evidence in [work issue #8](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/8). |
