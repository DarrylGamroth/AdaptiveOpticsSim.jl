...
| HIL-GPU-001 | 7 | Compute-device identity, prepared direction batching, one device owner, and device-resident state are explicit | Core | partial | partial | The normative [Gate 7 clauses](execution-and-placement.md#gate-7-single-gpu-requirements) distinguish backend family, concrete compute device, batch compatibility, ownership, completion, residency, and promotion evidence. [`compute_device`](../../src/core/backends.jl) and the CUDA/AMDGPU extensions report family-qualified identities independently of semantic backend selection, while [`PathExecutionRequirements`](../../src/plant/event_composition.jl) exposes each Gate 6 group's backend, compute device, and full-optical requirement without assigning placement. Focused CPU/fake-device and maintained optional-backend checks cover identity, views, mismatch rejection, and allocation-free requirement lookup. [`direction_batch.jl`](../../src/atmosphere/direction_batch.jl) adds an explicit model-owned atmosphere capability, immutable source/layer/grid/backend/device/capacity contract, separate mutable device-resident geometry/output workspace, zero-allocation whole-batch preflight, exact serial CPU oracle, and one three-dimensional accelerator extraction submission per layer followed by an explicit completion barrier. [`direct_imaging_batch.jl`](../../src/optics/direct_imaging_batch.jl) separately adds an explicit native-Fraunhofer direct-science capability and immutable model/grid/radiometry/type/backend/device/FFT/product/sample signature. It retains one ordered `IntensityMap` view per physical direction or spectral leaf while one prepared stacked field/output workspace executes a single optical-axis FFT plan and an explicit completed intensity/periodic-shift kernel. [`device_batching.jl`](../../src/plant/device_batching.jl) now binds compatible co-resident equal-rate physical `Source` paths to one unforgeable run-immutable owner with one exact backend execution context and stream, one atmosphere batch, one direct-imaging batch, canonical member storage, explicit same-device handoff to the pre-existing path products, and a completion barrier. Owned groups reject independent, duplicate, stale, or foreign submission through the existing Gate 6 claim lifecycle; CPU paths, unequal-rate paths, LGS/WFS paths, singletons, and incompatible models remain ordinary groups. [`plant_device_batching.jl`](../../test/testsets/plant_device_batching.jl) validates the CPU serial oracle, exact scheduler/epoch/RNG/command/disposition/acquisition state, common static aberration and selected-path NCPA isolation, held and delivered native-DM commands, equal-period phase/origin incompatibility, bounded topology, ownership failures, and conventional CMOS acquisition composition. The maintained optional matrix additionally covers integrated device residency, scalar-index prohibition, completed return, bounded host launch-descriptor allocation, and CPU parity. The broader WFS/detector model matrix and promoted first-use/service-cost evidence remain Gate 7 gaps; mixed placement and physical multi-device planning belong to Gates 9A/9B rather than this row. |
