...
|---|---:|---|---|---|---|---|
| HIL-PLANE-001 | 0 | A prepared telescope owns run-immutable aperture, reflectivity, spatial sampling, and geometry while each path owns its mutable pupil OPD/electric field and focal-plane result; telescope owns no temporal cadence or exposure duration | Core | partial | partial | [`Telescope`](../../src/optics/telescope.jl) has separated params/state foundations, but `TelescopeState` still combines path OPD, PSF products, and workspace and `TelescopeParams` owns `sampling_time`; structural ownership is tracked by [#10](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/10) and temporal removal by [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) |
| HIL-PLANE-002 | 0 | Caller-owned optical-plane products are separate from single-writer prepared propagation/spatial-filter workspaces, FFT plans, and scratch, with fixed prepared shape, numeric type, backend, and device | Core | partial | partial | [`ElectricField`](../../src/optics/electric_field.jl), [`SpatialFilter`](../../src/optics/spatial_filter.jl), and propagation helpers provide foundations but still combine data products and execution scratch; tracked by [work issue #10](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/10) |
| HIL-PLANE-003 | 0 | Static aberration and controllable-optic surfaces apply to explicit caller-owned path products without mutating shared telescope path state | Core | partial | partial | Surface formation exists for DMs, NCPA, and OPD maps, but maintained application paths mutate `tel.state.opd`; tracked by [work issue #10](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/10) |
| HIL-PLANE-004 | 0 | Every prepared optical product declares plane kind, dimensions, physical sampling, origin/centering, axis orientation, wavelength/channel, units/normalization, density-versus-cell-integrated measure, coherence/combination policy, numeric type, backend, and physical device where applicable; incompatible handoffs fail before repeated execution | Core | partial | partial | [`ElectricFieldParams`](../../src/optics/electric_field.jl) records resolution, wavelength, and sampling, but the complete plane/normalization/combination contract and cold-boundary validation are missing; established by [#10](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/10) and finalized by [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) |
