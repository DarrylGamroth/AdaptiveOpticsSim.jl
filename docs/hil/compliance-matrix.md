...
| HIL-CMD-001 | 4 | Every independently timed controllable optic or segment has an independent virtual-time command endpoint | Core | partial | partial | [`definitions.jl`](../../src/plant/definitions.jl) provides stable `ControllableOpticID` and `CommandEndpointID` values, one cold `ControllableOpticDefinition` per physical device, one-or-more explicitly owned endpoint schemas, declaration-order-independent lookup, and duplicate-owner rejection. [`plant_topology.jl`](../../test/testsets/plant_topology.jl) validates independent co-conjugated declarations, multi-endpoint ownership, ordinary acquisition-based feedback declaration, reorder invariance, structured failures, and fail-closed preparation. [`command_schemas.jl`](../../src/plant/command_schemas.jl) adds each endpoint's immutable semantic payload contract. Prepared endpoint state, virtual-time admission/application, and optical composition remain missing; the minimal HIL port arrives in Gate 4A. |
| HIL-CMD-002 | 4 | Validation, admission, effective time, application, hold, and terminal model disposition are distinct | Core | partial | partial | [`command_schemas.jl`](../../src/plant/command_schemas.jl) and [`plant_command_schemas.jl`](../../test/testsets/plant_command_schemas.jl) implement non-mutating payload compatibility as a named operation and explicitly exclude admission, sequencing, scheduling, clipping, application, and disposition. The mutable stages and atomic multi-optic latch remain later slices; atomicity is never inferred from placement. |
| HIL-CMD-003 | 4 | Future, late, rejected, and superseded commands use bounded explicit policies and never backdate plant state | Core | partial | partial | `CommandSequencePolicy` and `CommandEffectiveTimePolicy` provide typed duplicate/stale/reordered/skipped, future, late, and supersession vocabulary with invalid-combination checks. No calendar, capacity, no-backdating transition, or terminal disposition exists yet; those remain Gate 4 3/9. |
| HIL-CMD-004 | 4 | Every endpoint prepares a versioned core plant command schema covering payload type/shape, units, basis/calibration, absolute or incremental semantics, bounds, plant-effective-time policy, and duplicate/reordering behavior | Core | partial | partial | `PlantCommandSchema` is attached immutably to every declared endpoint and covers stable schema/version identity, exact scalar or backend-neutral array type/shape, units/sign, basis/revision, absolute/incremental semantics, uniform/unbounded values, and value/sequence/effective-time policies. Focused tests validate schema construction, payload compatibility, topology lookup, and uniqueness. Prepared endpoint ownership is still missing, so the requirement remains partial. HIL boundary descriptors, session correlation, timestamp mapping, leases, and outcome credit remain `HIL-PORT-005`. |
