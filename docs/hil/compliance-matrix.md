...
| HIL-PORT-003 | 4A | External integration declares adapter readiness, complete-product-to-first-observation lead time, and maximum lease hold time without promoting progressive fragments into canonical simulation events | HIL + user integration | implemented | validated | [`AdapterReadinessSnapshot` and `AdapterDeliveryContract`](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/blob/32c0db8d26d82b344d91ad68b8cc317e3104f1ad/src/ports/contracts.jl) make readiness, complete-product lead time, and positive maximum lease hold explicit orchestration data; serial arming rejects an adapter that is not ready. Focused [port tests](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/blob/32c0db8d26d82b344d91ad68b8cc317e3104f1ad/test/ports.jl) validate the contract, while the Gate 4A artifact separately records publication lateness, first observation, adapter-lead misses, and observed product-lease hold. Progressive fragments and first-packet timing remain user transport concerns rather than canonical simulation events. |
...
| HIL-LIFE-001 | 8 | Configure, prepare, arm, run, and stop/fail phases preserve ownership, adapter-readiness preconditions, prepared nonstructural state transitions, and bounded shutdown | HIL | planned | missing | Topology, schema, capacity, placement, and provider changes require another prepare/arm cycle. Delivery is tracked by [AdaptiveOpticsHIL #21](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/issues/21), [#22](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/issues/22), and [#24](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/issues/24). |
...
and its ordered work issues #18–#25. The requirement rows remain
`planned / missing` until merged implementation and maintained evidence justify
their independent state transitions.
