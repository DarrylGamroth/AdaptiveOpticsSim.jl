...
| HIL-TIME-003 | 4A | The first HIL vertical slice injects a monotonic-nanosecond `Clocks.AbstractNanoClock`, validating `CachedNanoClock` for deterministic tests and `SystemNanoClock` for production while core remains wall-clock independent | HIL | implemented | validated | AdaptiveOpticsHIL [`timing.jl`](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/blob/32c0db8d26d82b344d91ad68b8cc317e3104f1ad/src/timing.jl) binds one injected `Clocks.AbstractNanoClock` to an immutable execution-clock mapping, and [`timing.jl` tests](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/blob/32c0db8d26d82b344d91ad68b8cc317e3104f1ad/test/timing.jl) validate exact cached-clock replay, checked mapping, monotonic reads, and the system-clock provider. Serial arming requires adapter readiness before capturing the mapping. The core plant continues to accept explicit virtual time and has no dependency on HIL types. Epoch-domain mapping, cached-clock ownership/staleness, and operational lifecycle hardening remain Gate 8. |
...
| HIL-TIME-002 | 8 | External timestamp domains map into plant time with versioned offset, drift, uncertainty, and no retroactive remapping | HIL | planned | missing | User integration supplies synchronization observations. Delivery is tracked by [AdaptiveOpticsHIL #20](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/issues/20). |
