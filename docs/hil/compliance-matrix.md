...
| HIL-PLANE-004 | 0 | Every prepared optical product declares plane kind, coordinate domain, dimensions, sampling, origin/centering, axis orientation, wavelength/channel, units/normalization, density-versus-cell-integrated measure, coherence/combination policy, numeric type, backend, and physical device where applicable; incompatible handoffs fail before repeated execution | Core | implemented | partial | [`OpticalPlaneMetadata`](../../src/optics/planes.jl) now includes concrete photon-rate/dimensionless normalization, point-sampled/spatial-density/cell-integrated measure, and coherent/incoherent/non-combinable policy markers in addition to the structural, spectral, backend, and physical-device contract. Cold propagation, compatible accumulation, and [`prepare_detector_acquisition`](../../src/detectors/detector_acquisition.jl) reject incompatible metadata before repeated execution. [`Optical radiometry and combination contracts`](../../test/testsets/core_optics.jl), [`Detector`](../../test/testsets/detectors.jl), and the [optional backend matrix](../../test/backend_optional_common.jl) provide focused CPU and backend evidence; full available-hardware validation for this revision is pending in [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13). Names and units follow the maintained [`glossary`](../glossary.md). |
...
| HIL-ATM-002 | 0 | Atmosphere advancement accepts explicit model time or elapsed duration and never infers time from telescope sampling, detector cadence, wall clock, or a renderer | Core | implemented | partial | `advance_by!` and `advance_to!` in [`source_geometry.jl`](../../src/atmosphere/source_geometry.jl) are the maintained timed-model API. [`ClosedLoopRuntime`](../../src/control/runtime/types.jl) and single/grouped control-loop configurations require a finite positive `atmosphere_step`, preserve it across reconstruction changes, and pass it explicitly to one sensing-cycle atmosphere advance; detector exposure remains independent. Analytic wind-offset, monotonic-time, zero-duration, fixed-seed replay, shared-arm, invalid-step, and exact-runtime-step tests in [`atmosphere.jl`](../../test/testsets/atmosphere.jl) and [`control_and_runtime.jl`](../../test/testsets/control_and_runtime.jl) provide focused evidence; the complete validation matrix for [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) is pending. |
...
| HIL-RAD-001 | 0 | Physical detector-facing optical products are photon-arrival-rate products with declared units and density-versus-cell-integrated measure; telescope and optical front ends apply no elapsed-time factor, while detector acquisition integrates its explicit whole-exposure or incremental duration exactly once and never creates photons through declared spatial response and pixel integration; finite-support loss is explicit | Core | implemented | partial | Physical and normalized source radiometry in [`source.jl`](../../src/optics/source.jl) propagates through rate- or dimensionless-normalized fields and intensity maps without a telescope-time factor. [`DetectorAcquisitionPlan`](../../src/detectors/detector_acquisition.jl) validates a detector-facing map and prepares density-to-cell scaling and monochromatic QE; repeated whole or incremental capture applies a non-null presampling response before physical-pixel integration and applies explicit exposure exactly once. Diffractive Shack–Hartmann and Pyramid frame-detector paths fold sampled QE into each wavelength contribution before one incoherent sum and one acquisition; generic spectral capture retains its documented flux-weighted effective-QE contract. Finite response application uses zero extension, so signal leaving detector support is lost and edge samples are never amplified by clamping. [`core_optics.jl`](../../test/testsets/core_optics.jl), [`detectors.jl`](../../test/testsets/detectors.jl), [`shack_hartmann_and_sources.jl`](../../test/testsets/shack_hartmann_and_sources.jl), [`pyramid_bioedge_and_lgs.jl`](../../test/testsets/pyramid_bioedge_and_lgs.jl), and the [optional backend matrix](../../test/backend_optional_common.jl) cover unequal exposures, non-unit duration, density and cell-integrated fixtures, rejection, response order, component accumulation before one WFS acquisition, edge behavior, sampled-QE channel selection, prepared incremental GPU acquisition, and zero warmed allocation. Full available-hardware validation for [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) is pending. |
| HIL-RAD-002 | 0 | Spectral/source planes are accumulated elementwise only with compatible physical grids and declared incoherent intensity/rate semantics; otherwise a typed bundle or explicit prepared mapping is required | Core | implemented | partial | [`prepare_incoherent_sum`](../../src/optics/planes.jl) validates immutable metadata once and [`accumulate_intensity!`](../../src/optics/planes.jl) performs prepared allocation-free accumulation; [`OpticalProductBundle`](../../src/optics/planes.jl) preserves incompatible products without implicit conversion or resampling. [`Optical radiometry and combination contracts`](../../test/testsets/core_optics.jl) covers compatible accumulation and normalization, measure, policy, grid, and wavelength rejection; full available-hardware validation for [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) is pending. |
...
| HIL-WFS-003 | 0 | Optical front ends produce one or more detector-plane photon-arrival-rate products with an explicit spatial measure; presampling response, physical-pixel integration, QE and explicit elapsed-time integration, coupling, stochastic response, and readout follow optical formation and precede estimation, with one or multiple detector observations supported; the realized response's interior MTF remains available as a derived diagnostic | Core | partial | partial | Maintained WFS optical computations no longer apply telescope time, and the frame-detector path now applies presampling response before physical-pixel integration and exposure. [`detectors.jl`](../../test/testsets/detectors.jl) verifies response/integration/exposure order, [`shack_hartmann_and_sources.jl`](../../test/testsets/shack_hartmann_and_sources.jl) verifies spectral, extended-source, and asterism components are summed as optical rates before one acquisition, and [`pyramid_bioedge_and_lgs.jl`](../../test/testsets/pyramid_bioedge_and_lgs.jl) plus [`zernike_and_curvature.jl`](../../test/testsets/zernike_and_curvature.jl) verify detector-coupled exposure/QE normalization and fail-closed source support. The polychromatic SH branch still uses a reference-wavelength index-grid approximation rather than metadata-validated physical-grid resampling. A generic typed front-end contract, independent estimator stages, and multi-plane or multi-detector WFS composition remain later Gate 0 work; established in [#3](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/3) and proven with Curvature in [#6](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/6). |
| HIL-WFS-004 | 0 | Shack-Hartmann composes an independent microlens-array optical primitive, detector acquisition, and replaceable spot estimator while preserving geometric, diffractive, spectral, LGS, and asterism behavior | Core | planned | missing | [`ShackHartmannWFSState`](../../src/wfs/shack_hartmann/setup.jl) still combines lenslet propagation, detector-facing storage, calibration, and estimation. The maintained spectral/extended/asterism paths now sum component optical rates before one detector acquisition, providing an ordering oracle for the later decomposition; the spectral oracle retains the existing reference-wavelength index-grid approximation and does not claim physical-grid resampling. This is tracked by [work issue #4](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/4). |
...
| HIL-WFS-008 | 0 | Geometric and reduced-order WFS policies avoid unused diffractive workspace and declare whether they produce an approximate photon-arrival-rate product for detector processing or intentionally produce a direct measurement | Core | partial | partial | Geometric WFS modes and reduced slope foundations exist; the common prepared-stage and declared-validity contracts are missing; tracked across the [Gate 0 series](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/1) |
