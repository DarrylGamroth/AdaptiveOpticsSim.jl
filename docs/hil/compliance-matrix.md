...
| HIL-FID-003 | 4 | A reduced-order plant evolves time-correlated disturbances and maps correctly timed effective optic commands through calibrated path and sensor operators so commands causally change later products and a matched reference controller can close the loop | Plant + Control | implemented | validated | [`reduced_order.jl`](../../src/plant/reduced_order.jl) implements a deterministic harmonic disturbance, path projection, exact-schema command-response operators, sensor operator, mandatory calibration/envelope/omission metadata, fixed endpoint-slot resolution, and inspection of the current disturbance/residual. [`direct_measurement_events.jl`](../../src/plant/direct_measurement_events.jl) and [`event_composition.jl`](../../src/plant/event_composition.jl) exposure-average the held direct `WFSMeasurement`, preserve periodic or delivered-trigger start, readout/readiness and command phase causality, and bypass unused full-optical work. [`plant_reduced_order.jl`](../../test/testsets/plant_reduced_order.jl) proves exact replay, later-product command causality, clipped-effective-command visibility, modeled trigger-drop behavior, independent endpoint timing, schema dimensions/units/sign/basis/revision rejection, matched reference loop closure, expected wrong-sign/delay/stale/drop/calibration-mismatch degradation, full-path bypass, and the declared CPU allocation ceiling. The validated envelope is a small deterministic linear modal direct-measurement fixture; raw pixels, detector physics, tomography, external RTCs, integrated GPU event loops, full-optical equivalence, and instrument-scale performance remain unclaimed. |
| HIL-OPT-001 | 4 | Co-conjugated DMs remain independent devices even when prepared as one optical execution group | Plant + Optics | implemented | validated | [`definitions.jl`](../../src/plant/definitions.jl) preserves distinct physical declarations, required placement/visibility, and disjoint endpoint ownership. [`controllable_optics.jl`](../../src/plant/controllable_optics.jl) prepares immutable device plans with separate physical state/workspace plus canonical per-path co-placement groups, and [`event_composition.jl`](../../src/plant/event_composition.jl) applies each visible group additively in canonical optic-identity order without synchronizing independent endpoints. [`plant_command_composition.jl`](../../test/testsets/plant_command_composition.jl) proves one optic can update while another holds, their OPD effects add, explicit transactions remain separate from grouping, and declaration order does not alter optical products. [`plant_placed_optics.jl`](../../test/testsets/plant_placed_optics.jl) adds placement/group and selected-path reorder invariance while [`plant_conjugate_geometry.jl`](../../test/testsets/plant_conjugate_geometry.jl) preserves independent command state for compatible co-conjugated sampled-surface couplings. The [Gate 4 artifact](../../benchmarks/results/gate4/2026-07-24-command-plant.toml) retains independent multi-rate service-cost evidence. Native legacy-DM migration and device dynamics remain later work and are not claimed here. |
...
| HIL-OPT-002 | 5 | Common MCAO and path-specific MOAO planes support NGS and finite-height LGS footprints | Core | partial | partial | [`optic_placement.jl`](../../src/plant/optic_placement.jl) defines explicit pupil, atmospheric-conjugate, and focal placement plus all-path or selected-path visibility. [`controllable_optics.jl`](../../src/plant/controllable_optics.jl) resolves those declarations into bounded canonical per-path bindings and co-placed groups. [`pupil_footprint_coupling.jl`](../../src/plant/pupil_footprint_coupling.jl) adds a model-extension seam that composes source angle, finite-height LGS cone scale, conjugate altitude, explicit pupil-relay magnification/rotation/parity/decenter, and metric sampled-plane metadata into one immutable path-local mapping. Its CPU path uses direct finite-support bilinear accumulation; its accelerator path uses a backend-neutral kernel. [`event_composition.jl`](../../src/plant/event_composition.jl) binds each exact visible coupling while retaining independent optic state and canonical order. [`plant_placed_optics.jl`](../../test/testsets/plant_placed_optics.jl) validates the declaration foundation, and [`plant_conjugate_geometry.jl`](../../test/testsets/plant_conjugate_geometry.jl) validates analytic on/off-axis NGS and finite-height LGS footprints at multiple altitudes, registration/grid orientation, zero extension, interpolation, structured failures, compatible co-conjugated command isolation, inference, and zero warmed CPU application allocation. The optional backend matrix executes the identity and transformed sampled-surface paths device-resident with scalar indexing disabled. This row remains partial until native DM integration and complete common-plane MCAO plus path-specific MOAO scenarios pass; a generic sampled fixture alone is not an instrument-scale MCAO/MOAO claim. |
