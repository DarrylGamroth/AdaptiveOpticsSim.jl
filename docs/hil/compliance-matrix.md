...
| HIL-PATH-002 | 2 | Immutable optical-path definitions are separate from independently scheduled or triggered acquisition state | Core | partial | partial | [`definitions.jl`](../../src/plant/definitions.jl) introduces immutable, explicitly identified path and acquisition declarations; several acquisitions may reference one path, and model types must opt in to a cold configuration-only trait before either definition accepts them. [`plant_topology.jl`](../../test/testsets/plant_topology.jl) validates fan-out, reorder-invariant references, absent schedule/state, rejection of live detectors and mutable wrappers, duplicate IDs, and unknown references. Prepared compatible-path reuse, independent acquisition owners, and later scheduling remain missing. |
| HIL-API-001 | 2 | The core plant API separates run-immutable definitions, prepared plans, mutable single-writer state/workspaces, and caller-owned products without extending transitional OOPAO/frame-step abstractions or imposing HIL-only names on core models | Core | partial | partial | Gate 0 optical/WFS products already follow this split. The public [`PlantDefinition`, `OpticalPathDefinition`, and `AcquisitionDefinition`](../../src/plant/definitions.jl) now commit a HIL-neutral immutable topology boundary with typed lookup, structured validation, and an explicit fail-closed cold-model trait while excluding prepared plans, state, schedules, RNG, queues, and transport; [topology tests](../../test/testsets/plant_topology.jl) cover that boundary. General preparation and execution still must replace, rather than grow, `AOSimulation`, `ClosedLoopRuntime`, `CompositeControllableOptic`, and `RuntimeCommandLayout`; the [architecture index](../hil-package-boundary.md) and [plant specification](plant-and-optics.md) define the breaking target. |
