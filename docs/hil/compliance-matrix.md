...
| HIL-ATM-002 | 0 | Atmosphere advancement accepts explicit model time or elapsed duration and never infers time from telescope sampling, detector cadence, wall clock, or a renderer | Core | implemented | validated | `advance_by!` and `advance_to!` in [`source_geometry.jl`](../../src/atmosphere/source_geometry.jl) are the maintained timed-model API. Analytic wind-offset, monotonic-time, zero-duration, fixed-seed replay, and Gate 0 legacy-step tests verify the contract; timed atmosphere code contains no telescope sampling-time read. The transitional closed-loop adapter remains the caller that chooses its legacy duration until scheduling is refactored. |
