...
| HIL-GPU-001 | 7 | Compute-device identity, prepared direction batching, one device owner, and device-resident state are explicit | Core | partial | partial | The normative [Gate 7 clauses](execution-and-placement.md#gate-7-single-gpu-requirements) distinguish backend family, concrete compute device, batch compatibility, ownership, completion, residency, and promotion evidence. [`compute_device`](../../src/core/backends.jl) and the CUDA/AMDGPU extensions report family-qualified identities independently of semantic backend selection, while [`PathExecutionRequirements`](../../src/plant/event_composition.jl) exposes each Gate 6 group's backend, compute device, and full-optical requirement without assigning placement. Focused CPU/fake-device and maintained optional-backend checks cover identity, views, mismatch rejection, and allocation-free requirement lookup. [`direction_batch.jl`](../../src/atmosphere/direction_batch.jl) adds an explicit model-owned capability, immutable source/layer/grid/backend/device/capacity contract, separate mutable device-resident geometry/output workspace, zero-allocation whole-batch preflight, exact serial CPU oracle, and one three-dimensional accelerator extraction submission per layer followed by an explicit completion barrier. [`atmosphere_direction_batch.jl`](../../test/testsets/atmosphere_direction_batch.jl) covers finite/infinite NGS, finite-height LGS, science, extended-source, single-direction, stale/binding mismatch, nonmutation, fallback, and zero warmed CPU allocation. The [optional backend matrix](../../test/backend_optional_common.jl) covers finite/infinite parity, residency, scalar-index prohibition, device-ready observation, zero-allocation preflight, and a separate 64 KiB host launch-descriptor allocation ceiling. Wavelength/optical-product batching, the single backend resource owner, integrated detector/acquisition residency, and the complete promoted model/first-use/service-cost matrix remain Gate 7 gaps; mixed placement and physical multi-device planning belong to Gates 9A/9B rather than this row. |
