...
| HIL-FID-003 | 4 | A reduced-order plant evolves time-correlated disturbances and maps correctly timed effective optic commands through calibrated path and sensor operators so commands causally change later products and a matched reference controller can close the loop | Plant + Control | partial | missing | [`interaction_matrix.jl`](../../src/calibration/interaction_matrix.jl), [`reconstructors.jl`](../../src/control/reconstructors.jl), geometric WFS modes, and tomography primitives are foundations; the prepared reduced-order plant and acceptance evidence are missing |
...
transactions, event-loop composition, controller-output routing, and removal of
the superseded packed runtime are implemented. Reduced-order execution,
device-feedback acquisition, autonomous optical-device timing, and final
cross-backend closure remain.
