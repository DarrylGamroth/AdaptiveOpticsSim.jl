...
| HIL-OPT-002 | 5 | Common MCAO and path-specific MOAO planes support NGS and finite-height LGS footprints | Core | partial | partial | [`optic_placement.jl`](../../src/plant/optic_placement.jl) defines explicit pupil, atmospheric-conjugate, and focal placement plus all-path or selected-path visibility. [`controllable_optics.jl`](../../src/plant/controllable_optics.jl) resolves those declarations into bounded canonical per-path bindings and co-placed groups, and [`event_composition.jl`](../../src/plant/event_composition.jl) consumes direct visible ranges for pupil-plane execution. [`plant_placed_optics.jl`](../../test/testsets/plant_placed_optics.jl) validates ownership, errors, declaration-order invariance, grouping, selected-path isolation, inference, and zero warmed binding-access allocation. Atmospheric-conjugate full-optical preparation deliberately fails closed because NGS/LGS source-footprint transforms are not yet implemented; this row cannot advance to implemented/validated until those geometry and MCAO/MOAO scenarios pass. |
