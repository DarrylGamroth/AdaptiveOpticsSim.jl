...
|---|---:|---|---|---|---|---|
| HIL-PLANE-001 | 0 | A prepared telescope owns run-immutable aperture, reflectivity, spatial sampling, and geometry while each path owns its mutable pupil OPD/electric field and focal-plane result; telescope owns no temporal cadence or exposure duration | Core | partial | partial | [`TelescopeAperture`](../../src/optics/telescope.jl) now owns prepared geometry and independent [`PupilWavefront`](../../src/optics/planes.jl) products own path OPD/amplitude; [`Explicit optical products and surfaces`](../../test/testsets/core_optics.jl) verifies path isolation. `LegacyTelescopePathState` and `TelescopeParams.sampling_time` remain transitional gaps for [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13). |
| HIL-PLANE-002 | 0 | Caller-owned optical-plane products are separate from single-writer prepared propagation/spatial-filter workspaces, FFT plans, and scratch, with fixed prepared shape, numeric type, backend, and device | Core | implemented | validated | Caller-owned [`ElectricField` and `IrradiancePlane`](../../src/optics/planes.jl) products are separate from [`FraunhoferPropagation`/`FresnelPropagation`](../../src/optics/propagation.jl), [`DirectPSFWorkspace`](../../src/optics/psf.jl), and [`SpatialFilterWorkspace`](../../src/optics/spatial_filter.jl); the explicit-product tests enforce fixed preparation and zero warmed CPU allocation, frozen Gate 0 references verify numerical preservation, and the [`optional backend matrix`](../../test/backend_optional_common.jl) exercises device-resident products on maintained CUDA and AMDGPU hardware targets. |
...
| HIL-PLANE-004 | 0 | Every prepared optical product declares plane kind, dimensions, physical sampling, origin/centering, axis orientation, wavelength/channel, units/normalization, density-versus-cell-integrated measure, coherence/combination policy, numeric type, backend, and physical device where applicable; incompatible handoffs fail before repeated execution | Core | partial | partial | [`OpticalPlaneMetadata`](../../src/optics/planes.jl) declares the structural, spectral, backend, and physical-device contract—including concrete CUDA/ROCm device identifiers—and cold preparation rejects incompatible dimensions, sampling, origin/centering, orientation, wavelength, numeric type, backend, device, or destination; [`Optical-plane compatibility validation`](../../test/testsets/core_optics.jl) and the optional hardware matrix cover these boundaries. Physical normalization, spatial measure, and coherence fields intentionally remain explicit `Unspecified*` markers until [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) defines their semantics. |
| HIL-ATM-001 | 0 | One writer advances a shared atmosphere to a stable timed epoch; prepared path-local direction renderers consume that epoch in any order and write caller-owned outputs without shared mutable geometry caches or RNG mutation | Core | partial | partial | [`MultiLayerAtmosphere`](../../src/atmosphere/multilayer.jl) separates some advance/render work, but OPD and [`AtmosphereSourceGeometryCache`](../../src/atmosphere/source_geometry.jl) remain shared mutable state; tracked by [work issue #11](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/11) |
| HIL-ATM-002 | 0 | Atmosphere advancement accepts explicit model time or elapsed duration and never infers time from telescope sampling, detector cadence, wall clock, or a renderer | Core | planned | missing | Moving and infinite layers currently advance from [`tel.params.sampling_time`](../../src/atmosphere/multilayer.jl); tracked by [work issue #11](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/11) |
| HIL-SRC-001 | 0 | Source definitions are run-immutable execution descriptions; mutable profiles/images are frozen during preparation and direction, spectral, LGS, asterism, and extended-source expansion caches are prepared path-local state | Core | partial | partial | Ordinary source and spectrum definitions provide foundations, while [`ExtendedSource`](../../src/optics/extended_source.jl), mutable profile/image inputs, and atmosphere source geometry retain mutable caches or aliases; tracked by [work issue #11](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/11) |
| HIL-RAD-001 | 0 | Physical detector-facing optical products are photon rates with declared units and density-versus-cell-integrated measure; telescope and optical front ends apply no elapsed-time factor, while detector acquisition integrates its explicit whole-exposure or incremental duration exactly once and conserves flux through declared spatial response and pixel integration | Core | partial | partial | Source flux, optical formation, and detector exposure foundations exist, but [`fill_telescope_field!`](../../src/optics/electric_field.jl) multiplies by telescope sampling time before [`fill_frame!`](../../src/detectors/frame_response.jl) multiplies by detector exposure; tracked by [work issue #13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) |
| HIL-RAD-002 | 0 | Spectral/source planes are accumulated elementwise only with compatible physical grids and declared incoherent irradiance semantics; otherwise a typed bundle or explicit prepared mapping is required | Core | partial | partial | [`SpectralBundle`](../../src/optics/spectrum.jl) and spectral slices exist, but atmospheric spectral intensity currently accumulates arrays without a complete physical-grid compatibility contract; tracked by [work issue #13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) |
| HIL-SCI-001 | 0 | Native or prepared external science optics write caller-owned focal-plane photon-rate irradiance that one or more independent detector acquisitions integrate without telescope-owned PSF or cadence state | Core | partial | partial | [`compute_psf!`](../../src/optics/psf.jl), direct science products, shared optical arms, and the prepared PROPER seam are foundations; telescope-owned PSF/workspace, implicit time normalization, and coupled call sites remain gaps; tracked by [work issue #12](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/12) |
...
| HIL-WFS-002 | 0 | WFS implementations compose prepared optical-front-end, detector-acquisition, and estimator stages through dispatch over caller-owned products without HIL clock, scheduling, queue, or transport dependencies | Core | planned | missing | Current WFS state objects generally own all three stages; tracked by [work issue #3](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/3) |
| HIL-WFS-003 | 0 | Optical front ends produce one or more detector-plane photon-rate irradiance products; detector elapsed-time integration, MTF, pixel integration, coupling, stochastic response, and readout follow optical formation and precede estimation, with one or multiple detector observations supported | Core | partial | partial | Current detector-coupled paths preserve parts of the ordering, but telescope-time normalization remains coupled and no generic multi-plane or multi-detector WFS product contract exists; established in [#3](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/3) and proven with Curvature in [#6](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/6) |
| HIL-WFS-004 | 0 | Shack-Hartmann composes an independent microlens-array optical primitive, detector acquisition, and replaceable spot estimator while preserving geometric, diffractive, spectral, LGS, and asterism behavior | Core | planned | missing | [`ShackHartmannWFSState`](../../src/wfs/shack_hartmann/setup.jl) currently combines lenslet propagation, detector-facing storage, calibration, and estimation; tracked by [work issue #4](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/4) |
| HIL-WFS-005 | 0 | Pyramid and BioEdge compose independent physical focal-plane front ends, prepared modulation policies, detector acquisition, and calibrated differential estimators while sharing only semantically common machinery | Core | planned | missing | Current [Pyramid](../../src/wfs/pyramid/setup.jl) and [BioEdge](../../src/wfs/bioedge/setup.jl) states combine these responsibilities; tracked by [work issue #5](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/5) |
| HIL-WFS-006 | 0 | Zernike and Curvature compose independent optical front ends, detector acquisition, and estimators; Curvature supports both separate branch detectors and packed single-detector observations | Core | planned | missing | Current [Zernike](../../src/wfs/zernike.jl) and [Curvature](../../src/wfs/curvature.jl) states combine these responsibilities; tracked by [work issue #6](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/6) |
...
| HIL-WFS-008 | 0 | Geometric and reduced-order WFS policies avoid unused diffractive workspace and declare whether they produce approximate irradiance for detector processing or intentionally produce a direct measurement | Core | partial | partial | Geometric WFS modes and reduced slope foundations exist; the common prepared-stage and declared-validity contracts are missing; tracked across the [Gate 0 series](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/1) |
| HIL-WFS-009 | 0 | Every migrated stage preserves deterministic CPU results, maintained CUDA and AMDGPU parity, device residency, warmed allocation budgets, and comparable steady-state latency within predeclared absolute and relative gates | Core | partial | partial | Existing backend tests and hardware targets provide a foundation; decomposed-stage residency, allocation, and performance evidence is missing; final evidence is tracked by [work issue #8](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/8) |
...
| HIL-PLANT-001 | 2 | One coherent telescope and atmosphere epoch feeds every due path | Core | partial | partial | [`runtime/arms.jl`](../../src/control/runtime/arms.jl) and [shared-runtime tests](../../test/testsets/control_and_runtime.jl); immutable snapshots remain a gap |
| HIL-PATH-001 | 2 | Prepared paths own mutable propagation workspace; acquisitions separately own WFS, detector, readout, and publication state | Core | planned | missing | Required before path parallelism or safe result reuse |
...
| HIL-PATH-003 | 3 | Acquisitions over native NGS, finite-height LGS, and direct-science paths use independent virtual-time schedules | Core | partial | partial | [`source_geometry.jl`](../../src/atmosphere/source_geometry.jl), [`runtime/arms.jl`](../../src/control/runtime/arms.jl), and their [tests](../../test/testsets/control_and_runtime.jl); common multi-rate orchestration remains a gap |
| HIL-DET-001 | 3 | Exposure, optical samples, rolling shutter, frame transfer, nondestructive reads, readout, detector MTF/charge coupling, and complete-product publication have explicit composable event semantics | Core | partial | partial | [`detectors/interface.jl`](../../src/detectors/interface.jl) and [detector tests](../../test/testsets/detectors.jl); scheduled plant integration remains a gap |
...
| HIL-GPU-001 | 7 | Physical device identity, prepared direction batching, and device-resident state are explicit | Core | partial | partial | [CUDA](../../ext/AdaptiveOpticsSimCUDAExt.jl), [AMDGPU](../../ext/AdaptiveOpticsSimAMDGPUExt.jl), and [backend tests](../../test/backend_optional_common.jl); physical multi-device planning remains a gap |
...
|---:|---|---|
| 0 | Optical planes/radiometry, explicit atmosphere time, direct-science acquisition, and WFS stages decomposed and validated | planned |
...
- separate the telescope aperture/spatial-geometry definition from temporal
  cadence, caller-owned path OPD/electric-field/irradiance products, and
...
- establish photon-rate irradiance, detector-owned elapsed-time integration,
...
- form native or prepared external direct-science photon-rate irradiance
...
path mutation; and compatible direct-science rate irradiance supports
...
Detector elapsed-time integration, MTF, and sampling occur after optical photon-
rate irradiance formation and before estimation. Frozen complete products and
new stage products pass their declared tolerances; CPU steady-state allocation
...
- preserve detector MTF, charge-coupling, and readout-pipeline ordering when
...
including configured MTF/coupling behavior. Mixed rates produce the expected
...
- co-locate compatible consumers for field and PSF reuse
