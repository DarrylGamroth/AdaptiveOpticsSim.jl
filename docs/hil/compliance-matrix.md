...
| HIL-CMD-001 | 4 | Every independently timed controllable optic or segment has an independent virtual-time command endpoint | Core | partial | partial | [`definitions.jl`](../../src/plant/definitions.jl) now provides stable `ControllableOpticID` and `CommandEndpointID` values, one cold `ControllableOpticDefinition` per physical device, one-or-more explicitly owned endpoint identities, declaration-order-independent lookup, and duplicate-owner rejection. [`plant_topology.jl`](../../test/testsets/plant_topology.jl) validates independent co-conjugated declarations, multi-endpoint ownership, ordinary acquisition-based feedback declaration, reorder invariance, structured failures, and fail-closed preparation. Payload schemas, prepared endpoint state, virtual-time admission/application, and optical composition remain missing; the minimal HIL port arrives in Gate 4A. |
