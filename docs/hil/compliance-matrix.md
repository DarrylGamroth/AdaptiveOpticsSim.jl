...
| HIL-OPT-001 | 4 | Co-conjugated DMs remain independent devices even when prepared as one optical execution group | Plant + Optics | implemented | validated | [`definitions.jl`](../../src/plant/definitions.jl) preserves distinct physical declarations and disjoint endpoint ownership. [`controllable_optics.jl`](../../src/plant/controllable_optics.jl) prepares immutable device plans with separate physical state/workspace, and [`event_composition.jl`](../../src/plant/event_composition.jl) applies every default pupil-surface response additively in canonical optic-identity order without synchronizing independent endpoints. [`plant_command_composition.jl`](../../test/testsets/plant_command_composition.jl) proves one optic can update while another holds, their OPD effects add, explicit transactions remain separate from grouping, and declaration order does not alter optical products. The [Gate 4 artifact](../../benchmarks/results/gate4/2026-07-24-command-plant.toml) adds independent multi-rate service-cost evidence and an explicit atomic two-optic operation without conflating grouping and transaction semantics. The current default group is deliberately one common all-path co-conjugated group; path-local autonomous devices are excluded through an explicit execution role. Native legacy-DM migration, altitude placement, conjugate transforms, and general path-specific visibility remain later gates and are not claimed here. |
