...
| HIL-PORT-001 | 4A | Canonical command-submission, command-completion/outcome, and acquisition-completion ports are transport-neutral | HIL | implemented | validated | AdaptiveOpticsHIL [`Ports`](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/blob/eed6b861ae2022294722b323dcda05c78be2210c/src/ports.jl) owns concrete nonblocking command-submission, command-outcome, and complete-acquisition contracts without a transport dependency. [`ports.jl` tests](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/blob/eed6b861ae2022294722b323dcda05c78be2210c/test/ports.jl) distinguish transfer from semantic admission and cover full, empty, closed, rejection, correlation, terminal credit, ordered closure, and bounded drain behavior; the serial and benchmark evidence exercises all three ports together. Gates 8.1 and 8.2 harden the ring and canonical resource-lifecycle foundations without adding transport; execution-owner and run-coordinated failure/recovery remain later Gate 8 obligations. |
...
| HIL-LIFE-001 | 8 | Configure, prepare, arm, run, and stop/fail phases preserve ownership, adapter-readiness preconditions, prepared nonstructural state transitions, and bounded shutdown | HIL | partial | partial | Gate 8.4 [PR #29](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/pull/29) at merge [`c5b51eb`](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/commit/c5b51eb43f2cf13eec85ffa013981650c12c628a) adds the exact seven-state `RunPhase` transition matrix, positive run/session identity, inclusive wrap-safe arm windows, same-session execution-clock readiness, typed stop/terminal/failure events, immutable termination records, and typed configured/prepared/arming/armed/running serial handles. Configuration derives the exact event loop from the command bridge and preparation resolves run-bound acquisition storage through the HIL-neutral core seam delivered by [AdaptiveOpticsSim PR #125](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/pull/125) at [`2957a66`](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/commit/2957a666147f2ef83543de25ac637c14ae83ef09), preventing cross-prepared-plant product mixing. Arm validates quiescent ownership, stable clock identity, exact readiness timing, and the immutable active composition; clean stop validates its event and quiescence before committing terminal state, while runtime failure records a distinct failed termination. Lifecycle, serial, two-owner stress, benchmark-contract, Linux, Windows, verified Apple Silicon, coverage, and 100% Codecov project/patch gates passed before merge. The core exposes no prepared nonstructural acquisition/trigger/shutter/calibration/optic-mode control seam, so the companion accepts only an empty declaration rather than arbitrary callbacks. Long-lived owners, coordinated closure, failure acknowledgement, bounded drain, and ownership-deficit finalization remain [#22](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/issues/22) and [#24](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/issues/24); therefore this row is not promoted beyond `partial / partial`. |
...
| HIL-FAIL-001 | 8 | Required and optional work have declared shed, stop, failure, and recovery behavior without runtime provider substitution | HIL | planned | missing | Already admitted commands cannot disappear silently. Delivery is tracked by [AdaptiveOpticsHIL #23](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/issues/23). |
...
| HIL-EXEC-004 | 8 | Same-process runtime, adapter, and telemetry components have warmed allocation budgets and GC evidence, or allocating integration is isolated behind an explicit process boundary | HIL + user integration | planned | missing | The manifest records GC, process sharing, page policy, and soak evidence; GC is not disabled by assumption. Closure evidence is tracked by [AdaptiveOpticsHIL #25](https://github.com/DarrylGamroth/AdaptiveOpticsHIL.jl/issues/25). |
