...
|---|---:|---|---|---|---|---|
| HIL-PLANE-001 | 0 | A prepared telescope owns run-immutable aperture, reflectivity, spatial sampling, and geometry while each path owns its mutable pupil OPD/electric field and focal-plane result; telescope owns no temporal cadence or exposure duration | Core | partial | partial | [`TelescopeAperture`](../../src/optics/telescope.jl) owns prepared geometry, independent [`PupilFunction`](../../src/optics/planes.jl) products own path OPD/amplitude, and `TelescopeParams` no longer contains sampling time or exposure duration. [`Explicit optical products and surfaces`](../../test/testsets/core_optics.jl) verifies path isolation. `LegacyTelescopePathState` and convenience paths that still place mutable optical results on the telescope remain transitional gaps. |
...
| HIL-RAD-001 | 0 | Physical detector-facing optical products are photon-arrival-rate products with declared units and density-versus-cell-integrated measure; telescope and optical front ends apply no elapsed-time factor, while detector acquisition integrates its explicit whole-exposure or incremental duration exactly once and never creates photons through declared spatial response and pixel integration; finite-support loss is explicit | Core | implemented | validated | Physical and normalized source radiometry in [`source.jl`](../../src/optics/source.jl) propagates through rate- or dimensionless-normalized fields and intensity maps without a telescope-time factor. [`DetectorAcquisitionPlan`](../../src/detectors/detector_acquisition.jl) validates a detector-facing map and prepares density-to-cell scaling and declared-channel QE; repeated whole or incremental capture applies a non-null presampling response before physical-pixel integration and applies explicit exposure exactly once. Pyramid folds sampled QE into each wavelength contribution before one incoherent sum and one acquisition. Diffractive Shack–Hartmann provides the same ordering for components on one common wavelength grid and fails closed for distinct wavelength grids; generic spectral capture retains its documented flux-weighted effective-QE contract. Finite response application uses zero extension, so signal leaving detector support is lost and edge samples are never amplified by clamping. [`core_optics.jl`](../../test/testsets/core_optics.jl), [`detectors.jl`](../../test/testsets/detectors.jl), the prepared [`WFS stage contracts`](../../test/testsets/wfs_stage_contracts.jl), [`shack_hartmann_and_sources.jl`](../../test/testsets/shack_hartmann_and_sources.jl), [`pyramid_bioedge_and_lgs.jl`](../../test/testsets/pyramid_bioedge_and_lgs.jl), and the [optional backend matrix](../../test/backend_optional_common.jl) cover unequal exposures, non-unit duration, density and cell-integrated fixtures, rejection, response order, component accumulation before one WFS acquisition, edge behavior, sampled-QE channel selection, prepared incremental GPU acquisition, and zero warmed allocation. The stage fixture additionally proves one unchanged rate product fans out to unequal detector durations, presampling response precedes physical-pixel integration, and QE and duration are each applied exactly once. The full CPU suite and maintained AMDGPU and CUDA hardware matrices validate device residency and disabled scalar indexing. |
| HIL-RAD-002 | 0 | Spectral/source planes are accumulated elementwise only with compatible physical grids and declared incoherent intensity/rate semantics; otherwise a typed bundle or explicit prepared mapping is required | Core | implemented | validated | [`prepare_incoherent_sum`](../../src/optics/planes.jl) validates immutable metadata once and [`accumulate_intensity!`](../../src/optics/planes.jl) performs prepared allocation-free accumulation; [`OpticalProductBundle`](../../src/optics/planes.jl) preserves incompatible products without implicit conversion or resampling. Unspecified spectral coordinates cannot authorize an intensity sum, and diffractive Shack–Hartmann rejects distinct wavelength grids before state mutation instead of index-adding them. [`Optical radiometry and combination contracts`](../../test/testsets/core_optics.jl) covers compatible accumulation and normalization, measure, policy, grid, wavelength, and declaration rejection; the prepared [`WFS stage contracts`](../../test/testsets/wfs_stage_contracts.jl) retain distinct spectral/multi-plane leaves through independent acquisition mappings without index-wise addition; the Shack–Hartmann tests cover compatible same-grid execution and incompatible-grid rejection; the [optional backend matrix](../../test/backend_optional_common.jl) validates prepared accumulation and WFS-stage bundle residency on maintained AMDGPU and CUDA hardware targets. |
| HIL-SCI-001 | 0 | Native or prepared external science optics write a caller-owned focal-plane photon-arrival-rate product that one or more independent detector acquisitions integrate without telescope-owned PSF or cadence state | Core | partial | partial | Direct prepared field/propagation products now preserve photon-rate metadata and can feed independent prepared detector acquisitions without telescope cadence. [`Detector`](../../test/testsets/detectors.jl) verifies one immutable external or native rate map feeding independent unequal exposures, and [`Shared multi-arm optical runtime`](../../test/testsets/control_and_runtime.jl) verifies one science formation feeds unequal-exposure detectors without another atmosphere advance or path render. The telescope-owned convenience PSF/workspace, remaining coupled runtime call sites, and application-owned conversion of external results keep this row partial; tracked by [work issue #12](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/12). |
...
| HIL-WFS-002 | 0 | WFS implementations compose prepared optical-front-end, detector-acquisition, and estimator stages through dispatch over caller-owned products without HIL clock, scheduling, queue, or transport dependencies | Core | partial | partial | [`stage_contracts.jl`](../../src/wfs/stage_contracts.jl) defines typed caller-owned observations/measurements and six narrow prepared dispatch functions for optical formation, acquisition, and estimation, plus explicit acquired/direct measurement traits and structured preparation errors. [`wfs_stage_contracts.jl`](../../test/testsets/wfs_stage_contracts.jl) covers explicit pupil/field input, scalar/vector/matrix/stack wrapper construction, representative single- and multi-plane/detector execution, packed observations, direct measurement, inference, exact storage binding, and zero warmed CPU allocation; the [optional backend matrix](../../test/backend_optional_common.jl) exercises representative CUDA/AMDGPU residency with scalar indexing disabled. No clock, queue, transport, or runtime graph dependency is present. Maintained WFS family objects still combine stages internally until work issues [#4](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/4)–[#7](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/7), so the broader implementation requirement remains partial. |
| HIL-WFS-003 | 0 | Optical front ends produce one or more detector-plane photon-arrival-rate products with an explicit spatial measure; presampling response, physical-pixel integration, QE and explicit elapsed-time integration, coupling, stochastic response, and readout follow optical formation and precede estimation, with one or multiple detector observations supported; the realized response's interior MTF remains available as a derived diagnostic | Core | partial | partial | Maintained WFS optical computations no longer apply telescope time, and the frame-detector path applies presampling response before physical-pixel integration and exposure. The generic [`stage contract`](../../src/wfs/stage_contracts.jl) now requires detector-plane photon-arrival-rate products and supports static one/multiple-plane and one/multiple-observation composition. [`wfs_stage_contracts.jl`](../../test/testsets/wfs_stage_contracts.jl) proves rate fan-out to independent unequal exposures and a non-commuting response/integration fixture before typed estimation. [`detectors.jl`](../../test/testsets/detectors.jl) verifies the lower-level response/integration/exposure order, while maintained family tests retain their existing detector-coupled oracles. Diffractive Shack–Hartmann still rejects distinct wavelength grids until an explicit native-to-detector mapping exists; frozen legacy index-grid data is test-only characterization. Family-owned independent front ends and estimators, including Curvature's production separate/packed acquisition forms, remain work issues [#4](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/4)–[#6](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/6). |
| HIL-WFS-004 | 0 | Shack-Hartmann composes an independent microlens-array optical primitive, detector acquisition, and replaceable spot estimator while preserving geometric, diffractive, spectral, LGS, and asterism behavior | Core | planned | missing | [`ShackHartmannWFSState`](../../src/wfs/shack_hartmann/setup.jl) still combines lenslet propagation, detector-facing storage, calibration, and estimation. The maintained same-wavelength spectral, extended-source, and asterism paths sum compatible component optical rates before one detector acquisition, providing an ordering oracle for the later decomposition. Distinct-wavelength spectral support requires the planned independent microlens front end to prepare a flux-conserving mapping from each native angular grid to the detector grid; production fails closed until then. This is tracked by [work issue #4](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/4). |
| HIL-WFS-005 | 0 | Pyramid and BioEdge compose independent physical focal-plane front ends, prepared modulation policies, detector acquisition, and calibrated differential estimators while sharing only semantically common machinery | Core | planned | missing | Current [Pyramid](../../src/wfs/pyramid/setup.jl) and [BioEdge](../../src/wfs/bioedge/setup.jl) states combine these responsibilities. [`pyramid_bioedge_and_lgs.jl`](../../test/testsets/pyramid_bioedge_and_lgs.jl) now records the pre-decomposition oracle: optical photon rates are formed without telescope time, detector exposure/QE is applied once, incidence-normalized slopes preserve units, and unsupported source compositions fail structurally. Independent front ends and estimators remain tracked by [work issue #5](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/5). |
...
| HIL-WFS-008 | 0 | Geometric and reduced-order WFS policies avoid unused diffractive workspace and declare whether they produce an approximate photon-arrival-rate product for detector processing or intentionally produce a direct measurement | Core | partial | partial | `AcquiredObservationPath()` and `DirectMeasurementPath()` establish the acquired-observation versus intentional-direct-measurement distinction. The direct contract fixture consumes explicit pupil/field input, produces a typed measurement with declared units, remains fully inferred/allocation-free on CPU, and owns no fictitious rate, observation, or detector workspace. A separate full-physics versus approximate photon-rate fidelity declaration and migration of maintained geometric/reduced-order modes remain future work across the [Gate 0 series](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/1). |
| HIL-WFS-009 | 0 | Every migrated stage preserves deterministic CPU results, maintained CUDA and AMDGPU parity, device residency, warmed allocation budgets, and comparable steady-state latency within predeclared absolute and relative gates | Core | partial | partial | The representative prepared-stage compositions in [`wfs_stage_contracts.jl`](../../test/testsets/wfs_stage_contracts.jl) are inferred and allocation-free after CPU warmup. The shared [optional backend matrix](../../test/backend_optional_common.jl) covers device-resident optical formation, detector acquisition, copied/direct measurements, multi-plane/multi-detector fan-out, packed observations, and CPU/GPU rejection with scalar indexing disabled on maintained CUDA and AMDGPU targets. This is contract evidence, not complete migrated-family parity or latency-gate evidence; final evidence remains [work issue #8](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/8). |
