...
|---|---:|---|---|---|---|---|
| HIL-PLANE-001 | 0 | A prepared telescope owns run-immutable aperture, reflectivity, spatial sampling, and geometry while each path owns its mutable pupil OPD/electric field and focal-plane result; telescope owns no temporal cadence or exposure duration | Core | partial | partial | [`TelescopeAperture`](../../src/optics/telescope.jl) now owns prepared geometry and independent [`PupilFunction`](../../src/optics/planes.jl) products own path OPD/amplitude; [`Explicit optical products and surfaces`](../../test/testsets/core_optics.jl) verifies path isolation. `LegacyTelescopePathState` and `TelescopeParams.sampling_time` remain transitional gaps for [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13). |
...
| HIL-PLANE-004 | 0 | Every prepared optical product declares plane kind, coordinate domain, dimensions, sampling, origin/centering, axis orientation, wavelength/channel, units/normalization, density-versus-cell-integrated measure, coherence/combination policy, numeric type, backend, and physical device where applicable; incompatible handoffs fail before repeated execution | Core | partial | partial | [`OpticalPlaneMetadata`](../../src/optics/planes.jl) declares the structural, coordinate-domain, spectral, backend, and physical-device contract—including concrete CUDA/ROCm device identifiers—and cold preparation rejects incompatible plane kind, coordinate domain, dimensions, sampling, origin/centering, orientation, wavelength, numeric type, backend, device, or destination; [`Optical-plane compatibility validation`](../../test/testsets/core_optics.jl) and the optional hardware matrix cover these boundaries. Names and units follow the maintained [`glossary`](../glossary.md). Physical normalization, spatial measure, and coherence fields intentionally remain explicit `Unspecified*` markers until [#13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) defines their semantics. |
...
| HIL-RAD-001 | 0 | Physical detector-facing optical products are photon-arrival-rate products with declared units and density-versus-cell-integrated measure; telescope and optical front ends apply no elapsed-time factor, while detector acquisition integrates its explicit whole-exposure or incremental duration exactly once and conserves photons through declared spatial response and pixel integration | Core | partial | partial | Source photon irradiance, optical formation, and detector exposure foundations exist, but [`fill_telescope_field!`](../../src/optics/electric_field.jl) multiplies by telescope sampling time before [`fill_frame!`](../../src/detectors/frame_response.jl) multiplies by detector exposure; tracked by [work issue #13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) |
| HIL-RAD-002 | 0 | Spectral/source planes are accumulated elementwise only with compatible physical grids and declared incoherent intensity/rate semantics; otherwise a typed bundle or explicit prepared mapping is required | Core | partial | partial | [`SpectralBundle`](../../src/optics/spectrum.jl) and spectral slices exist, but atmospheric spectral intensity currently accumulates arrays without a complete physical-grid compatibility contract; tracked by [work issue #13](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/13) |
| HIL-SCI-001 | 0 | Native or prepared external science optics write a caller-owned focal-plane photon-arrival-rate product that one or more independent detector acquisitions integrate without telescope-owned PSF or cadence state | Core | partial | partial | [`compute_psf!`](../../src/optics/psf.jl), direct science products, shared optical arms, and the prepared PROPER seam are foundations; telescope-owned PSF/workspace, implicit time normalization, and coupled call sites remain gaps; tracked by [work issue #12](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/12) |
...
| HIL-WFS-003 | 0 | Optical front ends produce one or more detector-plane photon-arrival-rate products with an explicit spatial measure; detector elapsed-time integration, presampling response, pixel integration, coupling, stochastic response, and readout follow optical formation and precede estimation, with one or multiple detector observations supported; the corresponding MTF remains available as a derived diagnostic | Core | partial | partial | Current detector-coupled paths preserve parts of the ordering, but telescope-time normalization remains coupled and no generic multi-plane or multi-detector WFS product contract exists; established in [#3](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/3) and proven with Curvature in [#6](https://github.com/DarrylGamroth/AdaptiveOpticsSim.jl/issues/6) |
...
| HIL-DET-001 | 3 | Exposure, optical samples, rolling shutter, frame transfer, nondestructive reads, readout, presampling detector response, charge coupling, and complete-product publication have explicit composable event semantics; the response has a validated derived MTF where supported | Core | partial | partial | [`detectors/interface.jl`](../../src/detectors/interface.jl) and [detector tests](../../test/testsets/detectors.jl); scheduled plant integration remains a gap |
...
Detector elapsed-time integration, presampling response, and sampling occur
after optical photon-arrival-rate formation and before estimation. Frozen complete products and
