...
| HIL-CMD-002 | 4 | Validation, admission, effective time, application, hold, and terminal model disposition are distinct | Core | partial | partial | [`command_schemas.jl`](../../src/plant/command_schemas.jl) keeps non-mutating payload validation separate. [`command_admission.jl`](../../src/plant/command_admission.jl) adds distinct presentation, admission, pending, application-ready claim, applied/failed reporting, pending failure drain, and terminal-disposition values without mutating an optic. Requested effective, scheduled, admission, application-ready, and terminal timestamps use distinct accessors. [`plant_command_admission.jl`](../../test/testsets/plant_command_admission.jl) validates exactly-once terminal credits, copied payload ownership, stale/foreign/tampered-claim rejection, bounded drain, inference, and zero warmed CPU allocation. Effective optic mutation, state-dependent application bounds, held/safe state, silence, and atomic multi-optic latch remain later slices; atomicity is never inferred from placement. |
| HIL-CMD-003 | 4 | Future, late, rejected, and superseded commands use bounded explicit policies and never backdate plant state | Core | partial | partial | The standalone prepared endpoint implements fixed-capacity time ordering; accepted-sequence history; explicit duplicate/stale/reordered/skipped, future, late, capacity, and supersession outcomes; retry after nonadmission; apply-now lateness; and no-backdating. Absolute supersession emits one terminal credit per displaced pending presentation; incremental supersession is rejected as lossy. Focused tests validate deterministic scheduled-time/endpoint-ordinal/local-sequence order, capacity invariants, retry semantics, terminal outcomes, and terminal metadata derived from endpoint-owned state. Plant-event composition and fixed-arrival HIL overload evidence remain missing, so implementation and evidence stay partial at the system requirement level. |
| HIL-CMD-004 | 4 | Every endpoint prepares a versioned core plant command schema covering payload type/shape, units, basis/calibration, absolute or incremental semantics, bounds, plant-effective-time policy, and duplicate/reordering behavior | Core | partial | partial | `PlantCommandSchema` covers stable schema/version identity, exact scalar or backend-neutral array type/shape, units/sign, basis/revision, absolute/incremental semantics, uniform/unbounded values, and value/sequence/effective-time policies. `prepare_command_endpoint` binds one selected schema to fixed storage, ordering, and backend policy, and focused schema/admission tests validate the binding. `prepare_plant` does not yet prepare every declared optic endpoint or bind it to effective optic state, so the requirement remains partial. HIL boundary descriptors, session correlation, timestamp mapping, leases, and returned outcome credit remain `HIL-PORT-005`. |
| HIL-CMD-005 | 4 | Every endpoint declares replayable plant-time command-silence hold/safe/fail behavior, including the age origin and exact equal-time reset policy | Core | partial | partial | `CommandSilencePolicy` declares hold/safe/fail action, admission/application age origin, and a required positive plant-time timeout for safe/fail. Focused tests reject inconsistent policies. Prepared safe state, mutable age storage, and exact equal-time transition/reset behavior remain Gate 4 4/9; operational ingress liveness remains `HIL-LIFE-002`. |
