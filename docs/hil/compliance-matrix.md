...
| HIL-OPT-001 | 4 | Co-conjugated DMs remain independent devices even when prepared as one optical execution group | Core | partial | partial | [`definitions.jl`](../../src/plant/definitions.jl) and [`plant_topology.jl`](../../test/testsets/plant_topology.jl) now preserve two co-conjugated test-model devices as distinct physical declarations with disjoint endpoint ownership regardless of declaration order. [`controllable_optics.jl`](../../src/optics/controllable_optics.jl) and [runtime tests](../../test/testsets/control_and_runtime.jl) retain the additive composite numerical oracle. Prepared independent surface application and derived execution grouping remain missing. |
