...
|---|---|---|---|---|
| Telescope constructor surface | `Telescope.py` supports user-defined pupil, pupil reflectivity, display of optical path, petal-related flags, coronagraph property, and internal optical-path bookkeeping | `src/Optics/telescope.jl` currently models the core pupil/OPD/PSF path only; user pupil can be applied later with `set_pupil!`, but reflectivity maps, optical-path printing, petal flags, and coronagraph support are not part of the current core surface | Julia prioritized core simulation paths and avoided UI/state-display features in the hot path | Gap |
| Pupil reflectivity / flux map | OOPAO tracks `pupilReflectivity` and source `fluxMap` directly on the coupled telescope/source path | Julia computes the required amplitude from source flux and telescope geometry when propagating, but does not expose an equivalent reflectivity-map feature | Keeps the core simpler and backend-generic; reflectivity support can be added later as an optical element if needed | Gap |
...
| Area | Python behavior | Julia behavior | Why it differs | Status |
|---|---|---|---|---|
| Detector physics surface | OOPAO detector models integration buffering, sensor type (`CCD/CMOS/EMCCD`), gain, ADC bits, FWC saturation, dark current, background noise/map, and digitalization | `src/Optics/detector.jl` currently models QE, PSF sampling, binning, photon noise, and readout noise | Julia focused on the core simulation path needed for AO workflows and zero-allocation loops before adding full camera electronics | Gap |
| Integration buffering | OOPAO accumulates frames until `integrationTime` is reached | Julia detector is a one-call capture model with explicit `capture!` over a PSF | Simpler low-latency API; easier to use in deterministic loops | Gap |
| Output precision / quantization | OOPAO supports `bits`, `output_precision`, `digitalization` | Julia does not yet model detector quantization | Non-essential for core AO parity so far | Gap |
| Sensor-specific gain path | OOPAO distinguishes EMCCD vs CCD/CMOS behavior | Julia has no sensor-type branch | Same reason: keep the core detector minimal until required by a validated workflow | Gap |
