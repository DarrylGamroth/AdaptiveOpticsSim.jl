...
| Area | Python behavior | Julia behavior | Why it differs | Status |
|---|---|---|---|---|
| Detector physics surface | OOPAO detector models integration buffering, sensor type (`CCD/CMOS/EMCCD`), gain, ADC bits, FWC saturation, dark current, background noise/map, and digitalization | `src/detectors/` now models sensor type, gain ordering, ADC quantization, FWC saturation, dark current, explicit integration buffering, background flux/map handling, QE, PSF sampling, binning, photon noise, readout noise, and typed digital outputs | Julia added the detector-electronics path without changing the low-latency capture surface | Equivalent |
...
| Output type / quantization | OOPAO supports `bits`, `output_type`, `digitalization` | Julia models detector quantization levels via `bits`/`full_well` and now exposes a typed output buffer/return frame when digital output is requested | Same practical surface, but with an explicit internal analog frame plus typed output view | Equivalent |
...
| Detector-coupled exports | OOPAO often exposes detector-coupled frames through the owning wfs/camera object surface | Julia now makes the detector-export interface explicit through `output_frame(det)`, `DetectorExportMetadata`, `detector_export_metadata`, and `SimulationInterface` / `SimulationReadout` metadata accessors | Keeps detector physics separate from sensor/control objects while still making export conventions explicit for controller integration and analysis code | Intentional improvement |
...
| Area | Python behavior | Julia behavior | Why it differs | Status |
|---|---|---|---|---|
| Calibration storage | OOPAO uses class-heavy calibration helpers such as `ControlMatrix` and modal-basis utilities with print-driven status output | Julia ports these as typed data + explicit functions in `src/calibration/*.jl` | Better separation of operators, data, and state | Intentional |
| KL/NCPA default | OOPAO includes modal-basis and NCPA workflows, often around existing stored products and cockpit utilities | Julia now defaults NCPA KL basis generation to the HHt/PSD path and keeps DM-mode covariance (`MᵀM`) as an explicit fast option via `KLBasis(KLDMModes())` | The scientific default is now the more physical atmosphere-weighted path, while the lighter DM-centric path remains available when throughput matters more than fidelity | Intentional improvement |
...
| Compact closed-loop trace exports | OOPAO tutorials and saved traces usually communicate compact loop metrics as unlabeled NumPy arrays/matrices | Julia now exposes explicit row/container types for compact loop traces (`ClosedLoopTrace`, `GSCClosedLoopTrace`, `GSCAtmosphereReplayTrace`) while preserving the same compact metric surface | Same practical telemetry content, but with clearer exported semantics for downstream analysis and RTC harnesses | Intentional improvement |
