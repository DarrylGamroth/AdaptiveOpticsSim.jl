...
| Detector-coupled exports | OOPAO often exposes detector-coupled frames through the owning WFS/camera object surface | Julia now makes the detector-export boundary explicit through `output_frame(det)`, `DetectorExportMetadata`, `detector_export_metadata`, and `RTCBoundary` metadata accessors | Keeps detector physics separate from sensor/control objects while still making export conventions explicit for RTC and analysis code | Intentional improvement |
