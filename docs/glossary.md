...
| Event-driven detector acquisition | A prepared virtual-time lifecycle over one exact detector/acquisition-plan binding. `GlobalShutterAcquisitionDefinition` declares exposure, readout, and readiness durations; `PreparedGlobalShutterAcquisition` owns the fixed schedule and `GlobalShutterAcquisitionState` owns one mutable single-writer lifecycle. Begin, half-open interval accumulation, nondestructive read, close, readout completion, and readiness are separate transitions. |
...
| Readout completion / acquisition readiness | Readout completion finalizes the detector product at its declared plant timestamp. Acquisition readiness is a later or equal transition that permits the next exposure. A completed product does not imply that the detector has rearmed when a nonzero readiness delay is configured. |
