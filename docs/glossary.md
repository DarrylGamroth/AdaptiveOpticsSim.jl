...
| Prepared plant | `PreparedPlant`: the schedule-free result of preparing a `PlantDefinition`. It contains concrete tuples of path executors and acquisition owners but does not advance the atmosphere, decide due work, own a clock, or publish through a HIL port. |
| Prepared path executor | `PreparedPathExecutor`: the single-writer owner of one frozen source, exact plant-atmosphere binding, path-local pupil/field input, prepared atmosphere-materialization operation, acquisition-facing photon-rate result, and concrete prepared optical execution state. Its result may be borrowed read-only by several acquisitions. |
| Prepared acquisition owner | `PreparedAcquisitionOwner`: an independent detector/readout and optional WFS-estimator owner bound to one exact prepared path result. It owns caller-visible `AcquisitionProducts` and receives an explicit RNG for execution; it does not own the path workspace, schedule, trigger, queue, or transport. |
| Prepared acquisition selection | `PreparedAcquisitionSelection`: a cold, schedule-free selection of exact acquisition owners and their deduplicated optical paths, both canonicalized by stable declared identity so declaration and caller-selection order cannot alter direct serial order. It owns no RNG streams, cadence, trigger state, queue, transport, or retained atmosphere snapshot. |
...
| RNG owner identity | A stable run-unique identity for one stochastic state owner, derived from declared plant component identity rather than tuple position, task, thread, or device placement. Together with the run seed and derivation version it selects that owner's reproducible stream or addressable random domain. |
