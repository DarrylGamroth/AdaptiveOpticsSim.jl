...
| Optical-sample duration | The positive duration in seconds over which one detector-facing photon-arrival-rate sample contributes to an exposure. The current incremental convenience API names this `sample_duration`; it is not an absolute sample timestamp. A virtual-time scheduler owns the interval boundaries. |
