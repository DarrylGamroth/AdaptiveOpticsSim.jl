...
| Plant definition | `PlantDefinition`: the immutable binding of one telescope and atmosphere to reusable optical-path definitions and independent acquisition definitions. It is a topology declaration, not prepared execution, and owns no schedule, RNG stream, queue, transport, or HIL descriptor. Referenced scientific models are not thereby made deeply immutable. |
| Optical-path definition | `OpticalPathDefinition`: an explicitly identified source and cold optical-model declaration that can be prepared once and reused by compatible acquisitions. It owns no propagation workspace or acquisition state. |
| Acquisition definition | `AcquisitionDefinition`: an explicitly identified acquisition-model declaration plus a reference to one `OpticalPathID`. It is independent of the path definition and carries no schedule or prepared mutable acquisition state. |
