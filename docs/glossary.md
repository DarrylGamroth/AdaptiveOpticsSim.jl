...
| Prepared path executor | `PreparedPathExecutor`: the single-writer owner of one frozen source, path-local pupil/field input, acquisition-facing photon-rate result, and concrete prepared optical execution state. Its result may be borrowed read-only by several acquisitions. |
