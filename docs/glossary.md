...
| Controllable-optic definition | `ControllableOpticDefinition`: an explicitly identified, opted-in cold declaration of one physical controllable optic plus one or more `CommandEndpointID` values for independently timed or latched command surfaces. It owns no payload schema, mutable command/optic state, timing, placement, path visibility, optical execution group, or HIL descriptor. A device with several endpoint identities is still one physical optic; several optics never become one device merely because they are co-conjugated. |
...
| Prepared plant | `PreparedPlant`: the schedule-free result of preparing a currently supported `PlantDefinition`. It contains concrete tuples of path executors and acquisition owners plus exact stateful RNG streams derived from one run seed, derivation version, and stable owner identities. It does not advance the atmosphere, decide due work, own a clock, or publish through a HIL port. During the first Gate 4 topology slice, preparation rejects a nonempty controllable-optic set until prepared endpoint ownership is implemented; it never silently omits a declared device. |
...
| Prepared plant event loop | `PreparedPlantEventLoop`: the HIL-neutral virtual-time composition that binds periodic optical samples, periodic or delivered-trigger acquisition starts, detector lifecycle generators, one shared atmosphere timeline, stable owner RNGs, and caller-owned complete-product destinations. Its scheduler jumps directly to due timestamps; it owns no wall clock, task, queue, transport, lease, command endpoint, or RTC protocol. |
