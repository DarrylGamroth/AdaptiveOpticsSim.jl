...
| LiFT forward model | A prepared monochromatic focal-plane model that freezes the pupil transmission, modal OPD basis, diversity OPD, source photon irradiance, optional object kernel, observation-grid mapping, backend, and device. `PreparedLiFTForwardModel` owns reusable single-writer propagation state and predicts a cell-integrated photon-arrival rate; it owns no detector or acquisition cadence. |
