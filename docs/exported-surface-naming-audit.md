...
This note records a conservative naming audit of the exported public surface in
`AdaptiveOpticsSim.jl`.

The goal is not churn. The goal is to identify:
...
The exported surface is mostly in a good Julia/SciML shape now.
...
- avoid renaming working exports only for symmetry
- prefer adding explicit contract docs and conformance tests over introducing
  more aliases
...
- `InteractionMatrix`, `CalibrationVault`, `AOCalibration`
- `ShackHartmann`, `PyramidWFS`, `BioEdgeWFS`
...
### 1. `initialize_ao_shwfs`

Why it stands out:

- `shwfs` is less readable than `shack_hartmann`
- the function is a scenario builder rather than a core mathematical primitive

Conservative recommendation:

- do not rename yet
- if a later public-surface cleanup happens, prefer adding a clearer alias such
  as `initialize_ao_shack_hartmann` first
...
- keep for now
- reconsider together with `initialize_ao_shwfs`, not separately
...
Do not do a broad exported-surface rename pass now.
