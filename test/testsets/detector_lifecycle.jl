@testset "Detector lifecycle, readout, and output behavior" begin
    incremental_detector = Detector(exposure_duration=1.0,
    ...
    @test_throws InvalidConfiguration Detector(exposure_duration=0.0)
    @test_throws InvalidConfiguration Detector(exposure_duration=Inf)

    transition_values = fill(2.0, 2, 2)
    transition_map = detector_test_intensity_map(transition_values)
    transition_detector = Detector(exposure_duration=1.0,
        noise=NoiseNone(), qe=1.0, response_model=NullFrameResponse())
    ...
    glow_full_duration = Detector(exposure_duration=1.0, noise=NoiseNone(),
        qe=1.0, response_model=NullFrameResponse(),
        sensor=InGaAsSensor(glow_rate=20.0))
    glow_short_duration = Detector(exposure_duration=0.25,
        noise=NoiseNone(), qe=1.0, response_model=NullFrameResponse(),
        sensor=InGaAsSensor(glow_rate=20.0))
    ...
    det_qe_curve = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=qe_curve, binning=1,
    ...
    generalized_qe_detector = Detector(exposure_duration=1.0,
    ...
    det_tuple = Detector(exposure_duration=1.0, noise=(NoisePhoton(), NoiseReadout(0.5)),
        qe=1.0, binning=1)
    ...
    det_sat = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1, full_well=5.0)
    frame_sat = capture!(det_sat, fill(10.0, 4, 4); rng=MersenneTwister(2))
    @test maximum(frame_sat) == 5.0

    det_adc = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1, bits=8, full_well=10.0)
    ...
    det_adc_float = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
    ...
    det_adc_window_corr = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
    ...
    @test generalized_adc_window[1, :, :] == capture!(Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
            bits=8, full_well=100.0, readout_window=FrameWindow(2:5, 3:7), output_type=UInt16,
            sensor=HgCdTeSensor(sampling_mode=SingleRead()),
            response_model=NullFrameResponse(),
            correction_model=CompositeFrameReadoutCorrection((
    ...
    @test generalized_adc_window[2, :, :] == capture!(Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
            bits=8, full_well=100.0, readout_window=FrameWindow(2:5, 3:7), output_type=UInt16,
            sensor=HgCdTeSensor(sampling_mode=SingleRead()),
            response_model=NullFrameResponse(),
            correction_model=CompositeFrameReadoutCorrection((
    ...
    det_buffered = Detector(exposure_duration=2.0, noise=NoiseNone(), qe=1.0, binning=1)
    ...
    det_background_flux = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
        background_flux=2.0)
    frame_background_flux = capture!(det_background_flux, zeros(4, 4); rng=MersenneTwister(2))
    @test sum(frame_background_flux) > 0

    det_background_map = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
    ...
    det_stack = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=0.5, binning=1)
    ...
    allocation_stack_detector = Detector(exposure_duration=1.0,
        noise=NoiseNone(), qe=1.0, response_model=NullFrameResponse())
