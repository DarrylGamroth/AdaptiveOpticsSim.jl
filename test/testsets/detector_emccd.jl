...
@testset "EMCCD detector" begin
    ...
    det_emccd_base = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
        gain=1.0, sensor=EMCCDSensor())
    det_emccd_excess = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
    ...
    det_emccd_stochastic = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
    ...
    emccd_sequential = Detector(exposure_duration=1.0, noise=NoiseNone(),
    ...
    emccd_frame_transfer = Detector(exposure_duration=1.0,
    ...
    @test sequential_meta.frame_transfer_duration === nothing
    @test sequential_meta.sampling_read_duration == 0.016
    @test sequential_meta.sampling_acquisition_duration == 1.016
    @test sequential_meta.steady_state_frame_period == 1.016
    @test frame_transfer_meta.acquisition_mode == :frame_transfer
    @test frame_transfer_meta.frame_transfer_duration == 0.002
    @test frame_transfer_meta.sampling_read_duration == 0.016
    @test frame_transfer_meta.sampling_acquisition_duration == 1.018
    @test frame_transfer_meta.steady_state_frame_period == 1.002

    emccd_readout_limited = Detector(exposure_duration=0.001,
    ...
    emccd_unknown_timing = Detector(exposure_duration=1.0,
    ...
    @test unknown_timing_meta.sampling_read_duration === nothing
    @test unknown_timing_meta.sampling_acquisition_duration === nothing
    @test unknown_timing_meta.steady_state_frame_period === nothing

    det_emccd_conventional = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
    ...
    det_emccd_pc = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
        gain=10.0,
    ...
    det_emccd_efficiency = Detector(exposure_duration=1.0, noise=NoiseNone(),
        qe=1.0, gain=10.0,
    ...
    det_emccd_pc_batched = Detector(exposure_duration=1.0, noise=NoiseNone(), qe=1.0, binning=1,
...
@testset "EMCCD parameter, model, and saturation contracts" begin
    ...
    batched_gamma = Detector(exposure_duration=1.0, qe=1.0,
...
@testset "EMCCD multiplication qualification" begin
    ...
    exponential_detector = Detector(exposure_duration=1.0, qe=1.0,
    ...
    erlang_charge = 8.0
    erlang_gain = 5.0
    erlang_detector = Detector(exposure_duration=1.0, qe=1.0,
    ...
    fractional_detector = Detector(exposure_duration=1.0, qe=1.0,
    ...
    approximate_detector = Detector(exposure_duration=1.0, qe=1.0,
    ...
    cic_fourth = 24 * cic_mean * cic_gain^4 +
        3 * cic_variance^2
    cic_detector = Detector(exposure_duration=0.25, qe=1.0,
    ...
    prepared_detector = Detector(exposure_duration=0.25, qe=1.0,
        noise=NoiseNone(), gain=5.0,
        sensor=EMCCDSensor(excess_noise_factor=sqrt(2.0),
            multiplication_model=ConditionalGammaMultiplication()))
