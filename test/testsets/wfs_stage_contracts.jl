...
function contract_shack_hartmann_rate_values(spot_cube::AbstractArray,
    n_lenslets::Integer; gap::Integer=0, gap_value=0)
    n_sub = Int(n_lenslets)
    gap_px = Int(gap)
    n_axis_1, n_axis_2 = size(spot_cube, 2), size(spot_cube, 3)
    rate_values = similar(spot_cube,
        n_sub * n_axis_1 + (n_sub - 1) * gap_px,
        n_sub * n_axis_2 + (n_sub - 1) * gap_px)
    return WavefrontSensors._tile_shack_hartmann_spot_cube!(
        rate_values, spot_cube, n_sub; gap=gap_px,
        gap_value)
end
...
@testset "Prepared Zernike stages" begin
    ...
    bad_zernike_rates = (
        ...
        (contract_rate_map(copy(rate.values);
            ...
            :plane_metadata),
        (contract_rate_map(zeros(T, 3, 3);
            sampling=rate.metadata.sampling,
            coordinate_domain=NormalizedPupilCoordinates(),
            spectral=rate.metadata.spectral), :shape),
        (contract_rate_map(zeros(Float32, size(rate.values));
            sampling=Float32.(rate.metadata.sampling),
            coordinate_domain=NormalizedPupilCoordinates(),
    ...
    @test @inferred(form_wfs_optical_products!(rate, pupil, optics_plan)) === rate
    @test pupil.opd == pupil_before

    aliased_rate = contract_rate_map(
        sensor.front_end.propagation.workspace.nominal_frame;
        sampling=rate.metadata.sampling,
        coordinate_domain=NormalizedPupilCoordinates(),
        spectral=rate.metadata.spectral)
    alias_error = contract_captured_error() do
        prepare_wfs_optics(front_end, pupil, aliased_rate)
    end
    ...
    field = ElectricField(pupil, source;
        zero_padding=sensor.front_end.phase_spot.diffraction_padding, T=T)
    ...
    @test first_front_end.propagation.workspace !==
        second_front_end.propagation.workspace
    @test typeof(first_front_end.propagation.workspace) ===
        typeof(second_front_end.propagation.workspace)
    first_rate = zernike_rate_map(first_front_end, trajectory_pupil)
    second_rate = zernike_rate_map(second_front_end, trajectory_pupil)
    ...
    for step in 1:3
        ...
        form_wfs_optical_products!(first_rate, trajectory_pupil, first_plan)
        form_wfs_optical_products!(second_rate, trajectory_pupil, second_plan)
...
@testset "Prepared Curvature stages" begin
    ...
    prepared_field = prepare_wfs_optics(field_front_end, field,
        field_rates)
    @test @inferred(WavefrontSensors.wfs_optical_products(prepared_field)) === field_rates
    ...
    mkid = MKIDArrayDetector(exposure_duration=T(0.5), noise=NoiseNone(),
        sensor=MKIDArraySensor(qe=T(0.5), dark_count_rate=zero(T),
            ...
            T=T), T=T)
    missing_mkid_source_model = CurvaturePackedAcquisition(mkid;
        readout_model=CurvatureChannelReadout())
    missing_mkid_source_error = contract_captured_error() do
        prepare_wfs_acquisition(missing_mkid_source_model, rates,
            counting_observation)
    end
    @test missing_mkid_source_error isa WFSPreparationError
    @test missing_mkid_source_error.reason === :radiometry
    mkid_model = CurvaturePackedAcquisition(mkid;
        readout_model=CurvatureChannelReadout(),
        source=source)
    mkid_observation = WFSObservation(zeros(T, 2, 16);
        units=:photon_count, layout=:curvature_branch_channels)
    mkid_plan = prepare_wfs_acquisition(mkid_model, rates,
        mkid_observation)
    @test mkid_plan.source_throughput === zero(T)
    acquire_wfs_observation!(mkid_observation, rates, mkid_plan, rng)
    @test all(iszero, mkid_observation.storage)
...
@testset "Prepared physical Shack-Hartmann stages" begin
    ...
    @test propagation32 isa Optics.PreparedMicrolensPropagation
    @test Optics.microlens_propagation_plan(propagation32) isa
        Optics.MicrolensPropagationPlan
    @test Optics.microlens_propagation_workspace(propagation32) isa
    ...
    @test independent_propagation isa Optics.PreparedMicrolensPropagation
    ...
    WavefrontSensors.update_subaperture_layout!(independent_layout,
        pupil.amplitude .> zero(T), GeometryValidSubapertures(
            threshold=T(0.1), T=T))
    foreign_mla = MicrolensArray(; n_lenslets=4,
        diffraction_padding=2, n_pix_subap=4, half_pixel_shift=true, T=T)
    ...
    independent_workspace.intensity = copy(independent_workspace.intensity)
    ...
    @test @inferred(WavefrontSensors.wfs_optical_products(spectral_plan)) === spectral_rates
...
@testset "Prepared Pyramid and Bi-O-edge stages" begin
    ...
    reduced_observation = WFSObservation(zeros(T, reduced_side, reduced_side);
        units=:electron_count, layout=:four_pupil_mosaic)
    reduced_acquisition = prepare_wfs_acquisition(reduced_detector,
    ...
    for family in (Val(:pyramid), Val(:bi_o_edge))
        ...
        @test @inferred(WavefrontSensors.wfs_optical_products(spectral_plan)) ===
            spectral_rates
        form_wfs_optical_products!(spectral_rates, pupil, spectral_plan)
        @test sum(spectral_rates[1].values) /
        ...
        @test @inferred(WavefrontSensors.wfs_optical_products(path_plan)) === path_rates
...
@testset "Rate fan-out, bundles, and packed observations" begin
    ...
    @test @inferred(WavefrontSensors.wfs_optical_products(optics_bundle)) === bundle
...
@testset "Prepared WFS exact-target validation" begin
    ...
    @test @inferred(WavefrontSensors.wfs_optical_products(prepared_optics)) === rate
    ...
    @test @inferred(WavefrontSensors.wfs_optical_products(zernike_optics)) === zernike_rate
    @test WavefrontSensors._require_exact_wfs_target(
    ...
    @test @inferred(WavefrontSensors.wfs_optical_products(curvature_optics)) ===
        curvature_rates
    @test WavefrontSensors._require_exact_wfs_target(
    ...
    for family in (Val(:pyramid), Val(:bi_o_edge))
        ...
        @test @inferred(WavefrontSensors.wfs_optical_products(four_pupil_optics)) ===
            four_pupil_rate
        @test WavefrontSensors._require_exact_wfs_target(
