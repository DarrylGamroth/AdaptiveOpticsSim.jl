...
@testset "KA CPU Matrix" begin
    @testset "Core kernels" begin
        ...
        AdaptiveOpticsSim.WavefrontSensors.lift_copy_column!(
            SCALAR_CPU_STYLE,
        ...
        @test modal_two == modal_modes * modal_coefs
    ...
    @testset "Grouped and Shack-Hartmann stack kernels" begin
        ...
        grouped_wfs = ShackHartmannWFS(grouped_tel; n_lenslets=2, n_pix_subap=2)
        ...
        # The retained intensity and sampling stages consume a lenslet FFT
        # stack.  The retired field-stack kernels are superseded by the
        # explicit pupil/field kernels exercised below.
        fft_stack = zeros(ComplexF64, pad, pad, n_spots)
        fft_stack[2, 2, 1] = 2.0 + 0.0im
        fft_stack[3, 3, 4] = 2.0 + 0.0im
    ...
    @testset "Detector kernels" begin
        ...
        full_frame = reshape(collect(1.0:30.0), 5, 6)
        window = AdaptiveOpticsSim.Detectors.FrameWindow(2:4, 3:5)
        scalar_windowed_frame = zeros(3, 3)
        ka_windowed_frame = similar(scalar_windowed_frame)
        AdaptiveOpticsSim.Detectors._copy_windowed_frame!(
            SCALAR_CPU_STYLE, scalar_windowed_frame, full_frame, window)
        AdaptiveOpticsSim.Detectors._copy_windowed_frame!(
            KA_CPU_STYLE, ka_windowed_frame, full_frame, window)
        mark_ka_cpu_kernel!(:copy_windowed_frame_kernel!)
        @test ka_windowed_frame == scalar_windowed_frame == full_frame[2:4, 3:5]

        full_cube = reshape(collect(1.0:90.0), 5, 6, 3)
        scalar_windowed_cube = zeros(3, 3, 3)
        ka_windowed_cube = similar(scalar_windowed_cube)
        AdaptiveOpticsSim.Detectors._copy_windowed_cube!(
            SCALAR_CPU_STYLE, scalar_windowed_cube, full_cube, window)
        AdaptiveOpticsSim.Detectors._copy_windowed_cube!(
            KA_CPU_STYLE, ka_windowed_cube, full_cube, window)
        mark_ka_cpu_kernel!(:copy_windowed_cube_kernel!)
        @test ka_windowed_cube == scalar_windowed_cube == full_cube[2:4, 3:5, :]

        scalar_sampling_cube = zeros(5, 6, 2)
        ka_sampling_cube = zeros(5, 6, 2)
        AdaptiveOpticsSim.Detectors._copy_sampling_plane!(
            SCALAR_CPU_STYLE, scalar_sampling_cube, 2, full_cube, 3)
        AdaptiveOpticsSim.Detectors._copy_sampling_plane!(
            KA_CPU_STYLE, ka_sampling_cube, 2, full_cube, 3)
        mark_ka_cpu_kernel!(:copy_sampling_plane_kernel!)
    ...
    @testset "KA CPU kernel inventory" begin
        ...
        @test isempty(intersect(KA_CPU_EXERCISED_KERNELS, deferred))
        @test isempty(setdiff(all_kernels, classified))
        @test length(KA_CPU_EXERCISED_KERNELS) >= 40
    end

    @testset "Locally generated physical-frame fixture agreement" begin
        # Retired OOPAO slopes and the SPECULA index-grid characterization are
        # not detector-plane frame references.  Generate a local physical-frame
        # fixture instead, then compare the scalar and KA-CPU physical stages.
        root = mktempdir()
        create_reference_fixture(root)
        bundle = load_reference_bundle(root)
        cases = filter(case -> case.id == "shack_hartmann_frame",
            bundle.cases)
        @test length(cases) == 1
        case = only(cases)
        expected = load_reference_array(case)
        scalar_actual = adapt_reference_actual(case,
            compute_reference_actual(case))
        ka_actual = adapt_reference_actual(case,
            compute_reference_actual_ka_cpu(case))
        ...
        @test ka_cpu_close(ka_actual, scalar_actual; atol=case.atol,
            rtol=case.rtol)
    end
end
