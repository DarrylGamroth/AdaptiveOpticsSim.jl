...
include(joinpath(@__DIR__, "profile_control_loop_runtime.jl"))
...
function build_report()
    ...
    return Dict(
        ...
        "scope" => Dict(
            "artifact_kind" => "control_loop_orchestration_runtime",
            "backends" => ["cpu", "amdgpu", "cuda"],
            "families" => ["single_control_loop_runtime", "compatible_grouped_runtime", "mixed_grouped_runtime"],
...
function update_manifest!(artifact_path::AbstractString)
    ...
    push!(kept, Dict(
        "purpose" => "control-loop orchestration runtime artifact for PSP-15",
