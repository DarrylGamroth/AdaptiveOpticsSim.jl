...
_thermalized_sensor(sensor::EMCCDSensor, enabled::Bool, ::Type{T}) where {T<:AbstractFloat} =
    enabled ? EMCCDSensor(
        clock_induced_charge_per_frame=T(0.02),
...
function run_profile(; backend_name::AbstractString="cpu", config_dir::AbstractString=_config_dir_arg,
    ...
    println("  pupil_resolution: ", size(ctx.pupil.opd, 1))
    lenslets = AdaptiveOpticsSim.WavefrontSensors.n_lenslets(ctx.wfs)
    println("  n_lenslets: ", lenslets)
    println("  roi_size: ", div(size(ctx.rate_mosaic.values, 1), lenslets))
    println("  tiled_wfs_frame_shape: ", size(ctx.tiled_frame))
    println("  photon_rate_mosaic_shape: ", size(ctx.rate_mosaic.values))
    println("  observation_shape: ", size(observation_storage(ctx.observation)))
