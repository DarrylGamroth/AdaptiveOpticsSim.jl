...
function _resolve_backend(name::AbstractString)
    ...
    elseif lowered == "cuda"
        ...
        CUDA.functional() || error("profile_control_loop_runtime.jl requires a functional CUDA driver/device")
    ...
    elseif lowered == "amdgpu"
        ...
        AMDGPU.functional() || error("profile_control_loop_runtime.jl requires a functional ROCm installation and GPU")
...
function _control_loop_scale_config(name::AbstractString)
...
function _build_branch(::Type{T}, backend_selector, resolution::Int, wfs_samples::Int, n_act::Int,
    seed::Integer; sensor::Symbol=:sh, science::Bool=false, label::Symbol=Symbol("branch_", seed)) where {T<:AbstractFloat}
    sim, recon = _runtime_components(T, backend_selector, resolution, wfs_samples, n_act; sensor=sensor)
    wfs_det = Detector(noise=NoiseNone(), integration_time=T(1.0), qe=T(1.0), binning=1, T=T, backend=backend_selector)
    science_det = science ?
        Detector(noise=NoiseNone(), integration_time=T(1.0), qe=T(1.0), binning=1, T=T, backend=backend_selector) :
        nothing
...
function run_profile(; backend_name::AbstractString="cpu", scale_name::AbstractString="compact",
    ...
    cfg = _control_loop_scale_config(scale_name)
    ...
    single_branch = _build_branch(T, backend_selector,
        cfg.single_resolution, cfg.single_wfs_samples, cfg.single_dm_n_act, 11;
        sensor=:pyr, science=true, label=:main)
    build_t0 = time_ns()
    single = build_control_loop_scenario(single_cfg, single_branch)
    prepare!(single)
    single_build_time_ns = time_ns() - build_t0
    step!(single)
    _sync_backend!(backend_tag, command(single))
    ...
    compatible_branches = ntuple(i ->
        _build_branch(T, backend_selector,
    ...
    build_t0 = time_ns()
    compatible = build_control_loop_scenario(compatible_cfg, compatible_branches...)
    prepare!(compatible)
    compatible_build_time_ns = time_ns() - build_t0
    build_t0 = time_ns()
    mixed = build_control_loop_scenario(mixed_cfg, mixed_branches...)
    prepare!(mixed)
    mixed_build_time_ns = time_ns() - build_t0

    step!(compatible)
    step!(mixed)
    _sync_backend!(backend_tag, command(compatible))
    _sync_backend!(backend_tag, command(mixed))
    ...
    compatible_stack = grouped_wfs_stack(compatible)
    mixed_stack = grouped_wfs_stack(mixed)
    single_science = science_frame(single)
...
function print_profile(result)
    println("control_loop_runtime_profile")
    for key in (
        :backend,
        :scale,
        :single_wfs_family,
