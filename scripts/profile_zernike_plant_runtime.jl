"""
Profile the retained AdaptiveOpticsSim Zernike plant path:

    pupil field -> phase-spot pupil photon-rate map -> detector observation
...
function prepare_zernike_plant_profile(backend::AbstractArrayBackend)
    T = Float32
    resolution = 64
    telescope = Telescope(
...
@inline function step_zernike_plant_profile!(prepared)
...
@inline function replay_zernike_plant_profile!(prepared, steps::Int)
    @inbounds for _ in 1:steps
        step_zernike_plant_profile!(prepared)
    end
    return nothing
end
...
function run_zernike_plant_profile(backend)
    prepared = prepare_zernike_plant_profile(backend)
    replay_zernike_plant_profile!(prepared, PROFILE_WARMUPS)
    allocation_bytes = @allocated step_zernike_plant_profile!(prepared)
    ...
    if backend isa Backends.CPUBackend && !iszero(allocation_bytes)
        error("Zernike CPU plant step allocated " *
            "$allocation_bytes warmed Julia bytes")
    end

    start_nanoseconds = time_ns()
    _profile_region!(backend, "aos_zernike_plant") do
        replay_zernike_plant_profile!(prepared, PROFILE_STEPS)
    end
    elapsed_nanoseconds = time_ns() - start_nanoseconds
    values = Array(observation_storage(prepared.observation))
    all(isfinite, values) || error("Zernike plant produced non-finite values")
    sum(values) > 0 || error("Zernike plant produced an empty observation")

    println("zernike_plant_profile")
    ...
    if PROFILE_BACKEND == "cpu" && PROFILE_PRINT_CPU_SAMPLES
        Profile.clear()
        Profile.@profile replay_zernike_plant_profile!(prepared, PROFILE_STEPS)
        Profile.print(stdout; format=:flat, sortedby=:count)
    end
    return nothing
end

run_zernike_plant_profile(profile_backend(PROFILE_BACKEND))
