"""
Profile the retained AdaptiveOpticsSim Curvature plant path:

    pupil field -> paired-defocus photon-rate maps -> packed detector observation
...
for the registered FGA estimator. CPU runs also print a sampling profile; GPU
runs are intended to be launched under Nsight Systems/Compute or rocprofv3
after the functional gates pass.
"""
...
function prepare_curvature_plant_profile(backend::AbstractArrayBackend)
    T = Float32
    resolution = 64
    telescope = Telescope(
    ...
    observation = WFSObservation(
        similar(rates[1].values, T, 32, 16);
        units=:electron_count,
        layout=:curvature_branch_regions,
    )
    acquisition = prepare_wfs_acquisition(
        CurvaturePackedAcquisition(detector), rates, observation)
    return (; pupil, rates, observation, optics, acquisition,
        rng=Xoshiro(0x5a45524e))
end

@inline function step_curvature_plant_profile!(prepared)
    form_wfs_optical_products!(
        prepared.rates, prepared.pupil, prepared.optics)
...
@inline function replay_curvature_plant_profile!(prepared, steps::Int)
    @inbounds for _ in 1:steps
        step_curvature_plant_profile!(prepared)
    end
    return nothing
end

@inline _profile_region!(f::F, ::Backends.CPUBackend,
    ::AbstractString) where {F<:Function} = f()
...
function run_curvature_plant_profile(backend)
    prepared = prepare_curvature_plant_profile(backend)
    replay_curvature_plant_profile!(prepared, PROFILE_WARMUPS)
    allocation_bytes = @allocated step_curvature_plant_profile!(prepared)
    ...
    if backend isa Backends.CPUBackend && !iszero(allocation_bytes)
        error("Curvature CPU plant step allocated " *
            "$allocation_bytes warmed Julia bytes")
    end

    start_nanoseconds = time_ns()
    _profile_region!(backend, "aos_curvature_plant") do
        replay_curvature_plant_profile!(prepared, PROFILE_STEPS)
    end
    elapsed_nanoseconds = time_ns() - start_nanoseconds
    values = Array(observation_storage(prepared.observation))
    all(isfinite, values) || error("Curvature plant produced non-finite values")
    sum(values) > 0 || error("Curvature plant produced an empty observation")

    println("curvature_plant_profile")
    ...
    if PROFILE_BACKEND == "cpu" && PROFILE_PRINT_CPU_SAMPLES
        Profile.clear()
        Profile.@profile replay_curvature_plant_profile!(prepared, PROFILE_STEPS)
        Profile.print(stdout; format=:flat, sortedby=:count)
    end
    return nothing
end

run_curvature_plant_profile(profile_backend(PROFILE_BACKEND))
