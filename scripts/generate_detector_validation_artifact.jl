...
function apd_case()
    ...
    det = SPADArrayDetector(size(channels);
        integration_time=1.0,
        noise=NoiseNone(),
...
function spad_case()
    det = SPADArrayDetector((8, 8);
        integration_time=1.0,
        noise=NoiseNone(),
        sensor=SPADArraySensor(active_area_detection_efficiency=0.5, dark_count_rate=0.0, fill_factor=0.8),
    )
