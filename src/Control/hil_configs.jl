...
function AO188WFSDetectorConfig(;
    ...
    return AO188WFSDetectorConfig{T,typeof(convert_noise(noise, T)),typeof(sensor)}(
        T(integration_time),
        T(qe),
        psf_sampling,
        binning,
...
function AO1883kSurrogateParams(;
    T::Type{<:AbstractFloat}=Float32,
    diameter::Real=8.2,
    ...
    return AO1883kSurrogateParams{T,typeof(profile),typeof(support_model),typeof(branch_execution),typeof(replay_mode),typeof(high_detector),typeof(low_detector)}(
        T(diameter),
        T(sampling_time),
...
mutable struct AO1883kSurrogate{
    P,
    TEL,
    LOWTEL,
    ATM,
...
function _downsample_low_order_opd!(surrogate::AO1883kSurrogate)
...
function _measure_low!(surrogate::AO1883kSurrogate, rng::AbstractRNG)
...
function _measure_branches!(::TaskParallelBranchExecution, surrogate::AO1883kSurrogate)
...
function step!(surrogate::AO1883kSurrogate)
...
runtime_timing(surrogate::AO1883kSurrogate; kwargs...) = runtime_timing(() -> step!(surrogate); kwargs...)

function ao188_3k_phase_timing(surrogate::AO1883kSurrogate; warmup::Int=10, samples::Int=1000, gc_before::Bool=true)
