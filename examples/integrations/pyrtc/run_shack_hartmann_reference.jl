...
function prepare_pyrtc_components(pyrtc_root::AbstractString)
    ...
    return mktempdir() do temporary_directory
        ...
        slopes = SlopesProcess(python_config(
            pybuiltins,
            "type" => "SHWFS",
            "signalType" => "slopes",
            "subApSpacing" => 8,
...
function run_validation(
    ...
    prepared = prepare_hil_reference_system(:shack_hartmann)
    require_available_stream_names()
    components = nothing
    shared_memory = pyimport("multiprocessing.shared_memory")
    try
        components = prepare_pyrtc_components(pyrtc_root)
        flat_signal, interaction_matrix = calibrate_interaction_matrix!(
            components,
            prepared;
            poke,
        )
        length(flat_signal) == 104 || error(
            "expected 104 valid pyRTC SHWFS signals, got $(length(flat_signal))",
        )
        ...
        configure_pyrtc_loop!(components, interaction_matrix, gain)
        ...
        convergence_ratio < 1.0f-3 || error(
...
function main(args)
    ...
    result = run_validation(pyrtc_root)
    println("AOS/pyRTC Shack-Hartmann reference loop passed")
