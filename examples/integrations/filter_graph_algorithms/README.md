...
storage into a preallocated `(row=y, column=x)` FGA image, then uses FGA 0.5.2's
...
FGA 0.5.2 owns the calibrated Bi-O-edge differential signal. The boundary uses
...
`(row=y, column=x)` image. Registered FGA 0.5.2 then applies the selected pupil
...
passes directly without a copy or transpose. Registered FGA 0.5.2 owns the
